...
(()=>{
 ...
 async function bookingForm(box){
  ...
  if(!businessId)return;
  const form=document.createElement('form');form.className='doBookingForm';form.style.cssText='display:grid;gap:10px;margin-top:12px';receipt.append(form);
  const say=(a,e)=>ar()?a:e;
  const field=(label,type='select')=>{const wrap=document.createElement('label');wrap.style.cssText='display:grid;gap:5px';const caption=document.createElement('span');caption.textContent=label;const el=document.createElement(type==='select'?'select':'input');if(type!=='select')el.type=type;el.className='doCommandInput';el.required=true;wrap.append(caption,el);form.append(wrap);return el};
  ...
  const factsBox=document.createElement('div');factsBox.style.cssText='display:grid;gap:10px';form.append(factsBox);
  ...
  function requirements(){
   ...
   for(const name of req){
    ...
    const wrap=document.createElement('label');wrap.style.cssText='display:grid;gap:5px';const caption=document.createElement('span');caption.textContent=(ar()?definition.question_ar:definition.question_en)||({vehicle:say('نوع السيارة','Vehicle'),property_details:say('تفاصيل العقار','Property details'),location:say('موقع أرسله العميل عبر واتساب خلال ٢٤ ساعة','WhatsApp location received in the last 24 hours')})[name]||name;
    let input;
    if(name==='location'){
