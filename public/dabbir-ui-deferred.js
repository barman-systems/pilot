...
(()=>{
  ...
  let portfolio=null,loading=false,menuOpen=false,ready=false,initializing=null;
  ...
  async function init(){if(ready||initializing)return initializing;initializing=(async function(){addDictionary();ensureEntry();ensureSwitch();ensureScreen();patchRuntime();await getPortfolio(true);if(!portfolio)return;await restore();ready=true;refresh()})();try{await initializing}catch{tell(text().failed)}finally{initializing=null}}
...
})();const tick=()=>{if(!document.hidden&&document.querySelector('#screen-appointments.active'))window.dispatchEvent(new Event('dabbir:booking-view-changed'))};setInterval(tick,60000);window.addEventListener('focus',tick);document.addEventListener('visibilitychange',tick);})();
...
(()=>{
  ...
  function message(value,isError=false){const node=q('#dabbirActivityDetailsMsg');if(node){node.textContent=value||'';node.dataset.messageState=isError?'error':'normal'}}
...
(()=>{
 ...
 const msg=(value,error=false)=>{const node=card()?.querySelector('[data-message]');if(node){node.textContent=value;node.dataset.messageState=error?'error':'normal'}};
 function render(){
