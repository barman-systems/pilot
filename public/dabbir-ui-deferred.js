...
(()=>{
  ...
  const customer=x=>(data?.customers||[]).find(c=>c.id===x);
  ...
  const post=body=>api('',{method:'POST',body:JSON.stringify({business_id:id(),...body})});
  ...
  async function load(force=false){
    ...
    catch(error){q('#salonCalendarHost').innerHTML='<div class="salonError">'+esc(text().failed+' · '+error.message)+'</div>'}
    finally{loading=false}
  }
  function renderAllSalon(){if(!data)return;renderCalendar();renderToday();renderCustomers();renderScreens();}
  function rangeTitle(){if(view==='day')return fmt(cursor,{weekday:'long',day:'numeric',month:'long',year:'numeric'});if(view==='week'){const s=startWeek(cursor);return fmt(s,{day:'numeric',month:'short'})+' — '+fmt(plus(s,6),{day:'numeric',month:'short',year:'numeric'})}return fmt(cursor,{month:'long',year:'numeric'})}
  function calendarRows(){return (data?.appointments||[]).filter(a=>a.starts_at&&a.status!=='cancelled').sort((a,b)=>new Date(a.starts_at)-new Date(b.starts_at))}
  ...
  function eventHtml(a){const c=customer(a.customer_id),s=service(a.service_id),duration=Math.max(5,Math.round((new Date(a.ends_at)-new Date(a.starts_at))/60000)||s?.duration_minutes||60);return '<div class="salonEvent '+esc(a.status)+'" draggable="true" data-salon-appt="'+esc(a.id)+'"><b>'+esc(c?.display_name||text().customer)+'</b><small>'+esc(fmt(a.starts_at,{hour:'numeric',minute:'2-digit'}))+' · '+esc(ar()?(s?.name_ar||s?.name):(s?.name_en||s?.name))+'</small><small>'+esc(duration+' '+text().minutes+' · '+money(Number(a.quoted_price_aed||0)-Number(a.discount_aed||0)))+'</small><div class="salonEventActions"><button type="button" data-resize="-15" aria-label="-15">−</button><button type="button" data-resize="15" aria-label="+15">+</button><button type="button" data-open-appt="'+esc(a.id)+'">•••</button></div></div>'}
  function bindEvents(){
    ...
    qa('[data-resize]').forEach(btn=>btn.onclick=e=>{e.stopPropagation();const a=btn.closest('[data-salon-appt]'),row=(data.appointments||[]).find(x=>x.id===a.dataset.salonAppt);if(!row)return;const duration=Math.max(15,Math.round((new Date(row.ends_at)-new Date(row.starts_at))/60000)+Number(btn.dataset.resize));updateAppointment({action:'resize',appointment_id:row.id,duration_minutes:duration})});
  }
  ...
  function weekCalendar(){const start=startWeek(cursor),workers=(data.workers||[]).filter(w=>w.status==='active'),rows=calendarRows();let html='<div class="salonCalendarScroll"><div class="salonWeek">';for(let i=0;i<7;i++){const d=plus(start,i),dayRows=rows.filter(a=>key(a.starts_at)===key(d));html+='<div class="salonWeekDay"><div class="salonWeekHead">'+esc(fmt(d,{weekday:'short',day:'numeric',month:'short'}))+'</div>'+workers.map(w=>'<div class="salonWeekWorker"><span>'+esc(w.display_name)+'</span>'+(dayRows.filter(a=>a.worker_id===w.id).map(eventHtml).join('')||'<div class="salonGap">'+esc(text().availableGap)+'</div>')+'</div>').join('')+'</div>'}return html+'</div></div>'}
  function monthCalendar(){const first=new Date(cursor.getFullYear(),cursor.getMonth(),1),start=startWeek(first),rows=calendarRows(),today=key(new Date());let html='<div class="salonCalendarScroll"><div class="salonMonth">';for(let i=0;i<42;i++){const d=plus(start,i),items=rows.filter(a=>key(a.starts_at)===key(d));html+='<div class="salonMonthDay '+(key(d)===today?'today':'')+'"><div class="salonMonthDate">'+esc(String(d.getDate()))+'</div>'+items.slice(0,4).map(a=>'<button class="salonMonthEvent" data-open-appt="'+esc(a.id)+'">'+esc(fmt(a.starts_at,{hour:'numeric',minute:'2-digit'})+' · '+(worker(a.worker_id)?.display_name||''))+'</button>').join('')+(items.length>4?'<small>+'+(items.length-4)+'</small>':'')+'</div>'}return html+'</div></div>'}
  function renderCalendar(){
    const t=text(),host=q('#salonCalendarHost');if(!host)return;
    host.innerHTML='<div class="salonToolbar"><div class="salonToolbarGroup"><button data-cal-prev>‹</button><button data-cal-today>'+esc(t.today)+'</button><button data-cal-next>›</button></div><b>'+esc(rangeTitle())+'</b><div class="salonToolbarGroup"><button data-cal-view="day" class="'+(view==='day'?'on':'')+'">'+esc(t.day)+'</button><button data-cal-view="week" class="'+(view==='week'?'on':'')+'">'+esc(t.week)+'</button><button data-cal-view="month" class="'+(view==='month'?'on':'')+'">'+esc(t.month)+'</button><button class="on" data-quick-book>'+esc(t.quickBooking)+'</button></div></div><div class="salonCalendar">'+(view==='day'?dayCalendar():view==='week'?weekCalendar():monthCalendar())+'</div><p class="salonHint">'+esc(t.dragHint)+'</p>';
    q('[data-cal-prev]').onclick=()=>{cursor=view==='month'?new Date(cursor.getFullYear(),cursor.getMonth()-1,1):plus(cursor,view==='week'?-7:-1);load(true)};q('[data-cal-next]').onclick=()=>{cursor=view==='month'?new Date(cursor.getFullYear(),cursor.getMonth()+1,1):plus(cursor,view==='week'?7:1);load(true)};q('[data-cal-today]').onclick=()=>{cursor=new Date();load(true)};q('[data-quick-book]').onclick=openQuickBooking;
    qa('[data-cal-view]').forEach(btn=>btn.onclick=()=>{view=btn.dataset.calView;localStorage.setItem('dabbir_salon_calendar_view',view);load(true)});bindEvents();
  }
  async function moveAppointment(appointmentId,workerId,startsAt){const row=data.appointments.find(a=>a.id===appointmentId);if(!row)return;const duration=Math.max(5,Math.round((new Date(row.ends_at)-new Date(row.starts_at))/60000)||60);await updateAppointment({action:'move',appointment_id:appointmentId,worker_id:workerId,starts_at:startsAt,duration_minutes:duration})}
  ...
  async function saveRebook(appointmentId,days,value){const source=data.appointments.find(x=>x.id===appointmentId),next=value?new Date(value):plus(new Date(source.starts_at),days);try{await post({action:'rebook',appointment_id:appointmentId,starts_at:next.toISOString()});closeModal();notify(text().saved);await load(true)}catch(error){notify(text().failed+' · '+error.message)}}
