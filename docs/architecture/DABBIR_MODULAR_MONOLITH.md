...
## Slice 6: shared appointment interval construction

After #702 completed exact Production run 34473014165 attempt 1 on stable `67429babf48c0426157a2729ce444126752c5355`, the next slice fixes a proven calendar reschedule defect. Owner edits and Google/Outlook imports now use `_appointment-time-window.js::appointmentTimeWindow` to construct both endpoints. The owner duration helper remains unchanged; calendar preserves the positive persisted duration, including short and multi-day services. This consolidates interval arithmetic, not database writers or duration/authorization policy. See `RESCHEDULE_AUTHORITY_SLICE.md` and `reschedule-authority-proof.json` for contracts, callers, failing-before PostgreSQL evidence and the new Production gate.
