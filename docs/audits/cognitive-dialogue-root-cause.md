...
## Production contract mismatch after #676

Root cause: at 14:59 UTC on b21030c, the real Groq semantic calls returned HTTP 200 but both strict and JSON-object replies failed `REQUEST_SPAN_COUNT`. The application permits only zero, two or three independently quoted jobs; the provider schema allowed arrays of any length. Retrying the same underspecified contract consumed the bounded provider budget without producing a valid interpretation. Gemini also returned 429 and Cloudflare reached its configured timeout; these are separate observations, not explanations of the schema mismatch.

The generation schema now derives exact array cardinalities and quotation length bounds from the same constants as the existing application validator. Application acceptance is unchanged. This is independent of activity, language and message keywords. No model, prompt, provider, dependency, retry ceiling or authority gate changes.

Regression: the installed Zod JSON Schema reader independently evaluates the actual exported provider schema against 0/1/2/3/4/8 request spans and short/oversize evidence. Before the change it accepted one span while the application rejected it. After the change both reject it; the cognitive and golden conversations also pass. Production success still requires rerunning the protected conversations on the deployed SHA. Groq's documentation lists `anyOf` support but does not explicitly document array bound support, so runtime compatibility remains to be verified rather than inferred from documentation.

## Provider portability verification after #677

The production verification on f6a1da1 rejected the array-bound schema immediately with HTTP 400 / RESPONSE_FORMAT_PARAM. The compatibility request then still failed the old request-span count/evidence checks. Therefore #677 was not a successful production compatibility fix.

Use a documented object/null union for the wire representation: no independent jobs is null; multiple jobs require first_quote and second_quote, with third_quote nullable. A strict object cannot express one or four jobs. A validated adapter converts the quotes into the existing internal array before planning. Legacy arrays still have exactly the original accepted cardinalities and evidence limits. No pending-state, queue, tool or DB authority changes. The prompt's serialization instruction changes only to match this wire contract; no activity keyword or conversation-specific exception is added.

Tests retain rejection of legacy 1/4/8-span arrays, reject malformed/extra wire fields, verify 0/2/3 wire cardinalities with the installed independent JSON Schema reader, verify evidence bounds at the combined generation/application boundary, and exercise the actual interpreter adapter. Array/string length keywords are intentionally absent from the provider schema because the production endpoint rejected that request format; the application continues to enforce their limits. Retest the same required production cases, with unchanged deadlines and gates.

The fixed synthetic production probe now reports each model call's turn, proposed action/intent/risk/confidence, role and independent-request count. No quote, entity value, identifier or raw model text is added. This closes the diagnostic gap where transport/contract success was visible but a wrong semantic decision could only be inferred from final boolean checks. It does not alter probe acceptance or run another model call.
