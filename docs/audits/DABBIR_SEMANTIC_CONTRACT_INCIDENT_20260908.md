...
## Recurrent iPad navigation race found by end-to-end verification

On `e3d711252cb56e2001116d8538f623aca450a553`, workflow 34266241233 / job 102196362241 passed all three real semantic probes, including the actual CLARIFY policy decision (provider risk labels MEDIUM, LOW, MEDIUM). The iPad journey then timed out waiting for the Operations menu to remain open.

The navigation bridge paints Conversations immediately, refreshes the server asynchronously, then calls `showScreen` again. That last call closes the sidebar, including a menu the user has opened since the first paint. A deterministic deferred-promise regression reproduced this with the existing source: both successful and failed delayed refresh cases lost the open menu. The fix preserves a newly opened menu across the canonical render, while keeping normal navigation closure and stale-navigation epoch guards. The four race regressions now pass. No test timeout, click simulation, or production gate was relaxed.
