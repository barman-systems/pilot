...
begin
 ...
 insert into public.dabbir_businesses(id,slug,name,business_type,demo_mode) values(biz,'qa-brain-'||biz,'Understanding V2 rollback QA','services',true);
 ...
 insert into public.dabbir_services(id,business_id,name,name_ar,duration_minutes,price_aed,active) values(svc,biz,'QA services','استشارة تجريبية',60,50,true);
 ...
 result:=public.dabbir_activity_service_configure_v1(biz,br,svc,0,'{"activity_type":"services","delivery_modes":["AT_BUSINESS"]}');
 reset role;
 ...
 s:=s||jsonb_build_object('activity_contract_version',contract->>'contract_version','delivery_mode','AT_BUSINESS','entities',(s->'entities')||jsonb_build_object(
 'branch',jsonb_build_object('value',br,'source','DATABASE_FACT','confidence',1,'status','active'),
 'delivery_mode',jsonb_build_object('value','AT_BUSINESS','source','DATABASE_FACT','confidence',1,'status','active'),
 ...
 perform set_config('dabbir.smoke_result',jsonb_build_object('suite','brain-production-services-rollback-only','activity','services','delivery_mode','AT_BUSINESS','historical_context',true,'historical_revalidation',true,'canonical_availability',true,'read_result_persisted',true,'inferred_location_blocked',null,'missing_vehicle_blocked',null,'gps_readback',null,'provider_evidence','SYNTHETIC_FIXTURE_ONLY','context',true,'semantic_load',true,'commit',true,'duplicate_replay',true,'missing_mutation_blocked',true,'backdated_new_message_blocked',true,'availability',true,'booking_readback',true,'booking_idempotency',true,'verified_memory',true,'cancel_readback',true,'reschedule_readback',true)::text,true);
end $smoke$;
