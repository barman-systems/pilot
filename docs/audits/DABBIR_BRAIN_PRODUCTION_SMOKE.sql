...
begin
 ...
 insert into public.dabbir_businesses(id,slug,name,business_type,demo_mode) values(biz,'qa-brain-'||biz,'Understanding V2 rollback QA','car_wash',true);
 ...
 perform set_config('dabbir.smoke_result',jsonb_build_object('suite','brain-production-rollback-only','historical_context',true,'historical_revalidation',true,'canonical_availability',true,'read_result_persisted',true,'inferred_location_blocked',true,'missing_vehicle_blocked',true,'gps_readback',true,'provider_evidence','SYNTHETIC_FIXTURE_ONLY','context',true,'semantic_load',true,'commit',true,'duplicate_replay',true,'missing_mutation_blocked',true,'backdated_new_message_blocked',true,'availability',true,'booking_readback',true,'booking_idempotency',true,'verified_memory',true,'cancel_readback',true,'reschedule_readback',true)::text,true);
end $smoke$;
