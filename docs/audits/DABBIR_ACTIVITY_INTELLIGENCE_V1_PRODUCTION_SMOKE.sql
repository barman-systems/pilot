...
do $smoke$
declare biz uuid:=gen_random_uuid(); br uuid:=gen_random_uuid(); cust uuid:=gen_random_uuid(); conv uuid:=gen_random_uuid(); msg uuid:=gen_random_uuid(); batch uuid:=gen_random_uuid(); lock uuid:=gen_random_uuid(); svc uuid:=gen_random_uuid(); s jsonb; loaded jsonb; ctx jsonb; result jsonb; replay jsonb; blocked boolean:=false; own uuid:=gen_random_uuid(); contract jsonb; conn uuid:=gen_random_uuid();
begin
 ...
 result:=public.dabbir_activity_service_configure_v1(biz,br,svc,0,'{"activity_type":"car_wash","delivery_modes":["MOBILE"]}');
 reset role;
 ...
 s:=s||jsonb_build_object('activity_contract_version',contract->>'contract_version','delivery_mode','MOBILE','entities',(s->'entities')||jsonb_build_object(
 'branch',jsonb_build_object('value',br,'source','DATABASE_FACT','confidence',1,'status','active'),
 'delivery_mode',jsonb_build_object('value','MOBILE','source','DATABASE_FACT','confidence',1,'status','active'),
 ...
 update public.dabbir_ai_conversation_state set semantic_state=s#-'{entities,vehicle}' where business_id=biz and conversation_id=conv;
 blocked:=false;
 begin perform public.dabbir_semantic_execute_v2(batch,lock,1,'CREATE_BOOKING'); exception when others then if sqlerrm like 'ACTIVITY_REQUIRED_FACT_UNVERIFIED:vehicle%' then blocked:=true; else raise; end if; end;
 if not blocked then raise exception 'QA_MISSING_VEHICLE_NOT_BLOCKED'; end if;
 ...
 if not exists(select 1 from public.dabbir_appointments where business_id=biz and service_latitude=24.45 and service_longitude=54.37 and activity_intelligence->>'delivery_mode'='MOBILE') then raise exception 'QA_GPS_READBACK_FAILED'; end if;
