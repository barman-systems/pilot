...
if old not in agent:
    raise SystemExit('agent provider anchor not found')
agent = agent.replace(old, new, 1)
agent_path.write_text(agent)

Path('test/dabbir-cloudflare-ai.test.mjs').write_text(r'''import assert from 'node:assert/strict';
