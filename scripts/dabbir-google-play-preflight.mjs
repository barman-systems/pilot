...
const googleMigration = read('supabase/migrations/20260909080000_dabbir_google_entitlements_v1.sql');
...
requireStatic(/targetSdkVersion:\s*36/.test(appConfig), 'ANDROID_TARGET_SDK_36', 'Android target SDK is 36.');
...
requireStatic(eas?.submit?.production?.android?.track === 'internal', 'PLAY_INTERNAL_TRACK', 'Initial Google Play submission targets Internal testing.');
...
requireStatic(/\/api\/mobile\/auth\/login/.test(deletePage) && /\/api\/mobile\/account-delete/.test(deletePage) && /DELETE_DABBIR_ACCOUNT/.test(deletePage), 'ACCOUNT_DELETION_EXTERNAL_FUNCTIONAL', 'External deletion page invokes the real DABBIR deletion endpoint.');
...
requireStatic(/force row level security/i.test(googleMigration) && /revoke all on public\.dabbir_google_entitlements from public, anon, authenticated/i.test(googleMigration), 'GOOGLE_PLAY_TOKEN_SERVER_ONLY', 'Google purchase tokens are inaccessible to app/client database roles.');
...
const releaseExternalVerification = releaseMode && failures.length === 0 ? [
  ...
  { code: 'PLAY_APP_CONTENT_FORMS', detail: 'Complete applicable Play Console App content forms.' },
  { code: 'SIGNED_AAB_INTERNAL_TEST_VERIFICATION', detail: 'Confirm the final signed AAB in Google Play Internal testing.' }
] : [];
...
const report = {
  ...
  verdict: failures.length ? 'FAIL' : releaseConfigOnly ? 'RELEASE_CONFIG_PASS_EXTERNAL_VERIFICATION_REQUIRED' : (external.length ? 'INTERNAL_PASS_EXTERNAL_BLOCKED' : 'PASS'),
