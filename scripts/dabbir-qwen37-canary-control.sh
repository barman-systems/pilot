...
# First force the project configuration OFF. Existing deployments do not change,
# so this cannot activate traffic. A fresh OFF deployment is established below.
upsert_env 0 0
...
# For activation, require a Vercel Production source built from this exact main
# SHA, then redeploy it once while OFF. This OFF deployment is the rollback point
# and contains the same code as the eventual 1% activation.
...
# Only after the exact-code OFF rollback point is live do we enable the bounded
# 1% project configuration and create a fresh deployment to consume it.
upsert_env 1 1
