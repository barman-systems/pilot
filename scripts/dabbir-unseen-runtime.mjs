...
const result=[];
// Supplement the frozen corpus with a tool-result oracle. The first turn
// supplies service/time but no date, so both dates can be checked safely only
// after the customer's explicit conditional instruction on the second turn.
const cases=process.argv.includes('--require-conditional-fallback')?[...unseenSpec,
 ['strict/conditional-empty-availability','salon',['أبي قص شعر الساعة 18:00','إذا ما فيه اليوم شوف باجر'],{goal:'BOOK_SERVICE',service:'first',time:'18:00'}],
]:unseenSpec;
for(const spec of cases){
 ...
 for(const [i,body] of [...item.input_history,item.current_message].entries()){
  ...
  const rpc=async(name,args)=>{
   ...
   if(name==='dabbir_semantic_check_availability_v1'){toolCalls.push({turn:i,action:'CHECK_AVAILABILITY',date:state.entities?.date?.value??null,verified:true});return {slots:[]};}
 ...
 // A correct first date is insufficient for "today, otherwise tomorrow".
 // This optional stricter oracle examines the tool sequence after an empty
 // availability result. It is intentionally independent of the reducer shape.
 if(item.id==='strict/conditional-empty-availability'){
  const dates=toolCalls.filter(x=>x.action==='CHECK_AVAILABILITY').map(x=>x.date);
  const expected=['2026-09-10','2026-09-11'];
  checks.push({field:'conditional_availability_dates',expected,actual:dates,pass:JSON.stringify(dates)===JSON.stringify(expected)});
 }
...
report.benchmark_variant=process.argv.includes('--require-conditional-fallback')?'FROZEN_104_PLUS_CONDITIONAL_TOOL_RESULT_1':'FROZEN_104';
report.runner_sha256=crypto.createHash('sha256').update(fs.readFileSync(new URL(import.meta.url))).digest('hex');
report.metrics.conditional_availability_sequence=metric(all.filter(c=>c.field==='conditional_availability_dates'));
