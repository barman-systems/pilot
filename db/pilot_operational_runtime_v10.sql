...
create policy pilot_businesses_member_select
on public.pilot_businesses
for select
to authenticated
using (pilot_private.has_permission(id,'view_business'));

-- AI reply gate must execute under caller RLS and caller permissions.
create or replace function public.pilot_ai_may_reply(p_business_id uuid, p_conversation_id uuid)
returns boolean
language sql
stable
security invoker
set search_path to 'public','pg_temp'
as $$
  select pilot_private.has_permission(p_business_id,'reply_conversations')
    and exists(
      select 1 from public.pilot_conversations c
      where c.id=p_conversation_id
        and c.business_id=p_business_id
        and c.state in ('ai_active','waiting_customer','action_required')
    )
    and not exists(
      select 1 from public.pilot_handoffs h
...
revoke all on function public.pilot_ai_may_reply(uuid,uuid) from public;
revoke all on function public.pilot_ai_may_reply(uuid,uuid) from anon;
grant execute on function public.pilot_ai_may_reply(uuid,uuid) to authenticated;
