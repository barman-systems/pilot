...
alter table public.pilot_memberships add constraint pilot_memberships_role_check
...
alter table public.pilot_businesses add constraint pilot_businesses_business_type_check
...
begin
 ...
 if tg_op='UPDATE' and old.role='owner' and exists(select 1 from public.pilot_businesses b where b.id=old.business_id and b.owner_id=old.user_id) and (new.business_id<>old.business_id or new.user_id<>old.user_id or new.role<>'owner') then raise exception 'BUSINESS_OWNER_MEMBERSHIP_IMMUTABLE'; end if;
...
revoke update,delete on public.pilot_messages from authenticated;
revoke all on public.pilot_verification_challenges from anon,authenticated;

-- Business and team RBAC.
drop policy if exists pilot_businesses_member_update on public.pilot_businesses;
create policy pilot_businesses_member_update on public.pilot_businesses for update to authenticated using(pilot_private.has_permission(id,'manage_business')) with check(pilot_private.has_permission(id,'manage_business'));
...
create policy pilot_memberships_team_select on public.pilot_memberships for select to authenticated using(user_id=(select auth.uid()) or pilot_private.has_permission(business_id,'manage_team'));
create policy pilot_memberships_team_insert on public.pilot_memberships for insert to authenticated with check(pilot_private.can_manage_role(business_id,role));
create policy pilot_memberships_team_update on public.pilot_memberships for update to authenticated using(pilot_private.can_manage_role(business_id,role)) with check(pilot_private.can_manage_role(business_id,role));
create policy pilot_memberships_team_delete on public.pilot_memberships for delete to authenticated using(pilot_private.can_manage_role(business_id,role));

-- Customers.
drop policy if exists pilot_customers_member_all on public.pilot_customers;
create policy pilot_customers_select on public.pilot_customers for select to authenticated using(pilot_private.has_permission(business_id,'view_customers'));
create policy pilot_customers_insert on public.pilot_customers for insert to authenticated with check(pilot_private.has_permission(business_id,'edit_customers'));
create policy pilot_customers_update on public.pilot_customers for update to authenticated using(pilot_private.has_permission(business_id,'edit_customers')) with check(pilot_private.has_permission(business_id,'edit_customers'));
create policy pilot_customers_delete on public.pilot_customers for delete to authenticated using(pilot_private.has_permission(business_id,'edit_customers'));

-- Conversations/messages.
drop policy if exists pilot_conversations_member_all on public.pilot_conversations;
create policy pilot_conversations_select on public.pilot_conversations for select to authenticated using(pilot_private.has_permission(business_id,'view_conversations'));
create policy pilot_conversations_insert on public.pilot_conversations for insert to authenticated with check(pilot_private.has_permission(business_id,'reply_conversations'));
create policy pilot_conversations_update on public.pilot_conversations for update to authenticated using(pilot_private.has_permission(business_id,'reply_conversations')) with check(pilot_private.has_permission(business_id,'reply_conversations'));

drop policy if exists pilot_messages_member_all on public.pilot_messages;
create policy pilot_messages_select on public.pilot_messages for select to authenticated using(pilot_private.has_permission(business_id,'view_conversations'));
create policy pilot_messages_insert on public.pilot_messages for insert to authenticated with check(pilot_private.has_permission(business_id,'reply_conversations'));

-- Appointments.
drop policy if exists pilot_appointments_member_all on public.pilot_appointments;
create policy pilot_appointments_select on public.pilot_appointments for select to authenticated using(pilot_private.has_permission(business_id,'view_appointments'));
create policy pilot_appointments_insert on public.pilot_appointments for insert to authenticated with check(pilot_private.has_permission(business_id,'manage_appointments'));
create policy pilot_appointments_update on public.pilot_appointments for update to authenticated using(pilot_private.has_permission(business_id,'manage_appointments')) with check(pilot_private.has_permission(business_id,'manage_appointments'));
create policy pilot_appointments_delete on public.pilot_appointments for delete to authenticated using(pilot_private.has_permission(business_id,'manage_appointments'));

-- Channels.
drop policy if exists pilot_channels_member_all on public.pilot_channels;
create policy pilot_channels_select on public.pilot_channels for select to authenticated using(pilot_private.has_permission(business_id,'view_integrations'));
create policy pilot_channels_insert on public.pilot_channels for insert to authenticated with check(pilot_private.has_permission(business_id,'manage_integrations'));
create policy pilot_channels_update on public.pilot_channels for update to authenticated using(pilot_private.has_permission(business_id,'manage_integrations')) with check(pilot_private.has_permission(business_id,'manage_integrations'));
create policy pilot_channels_delete on public.pilot_channels for delete to authenticated using(pilot_private.has_permission(business_id,'manage_integrations'));

-- Services and knowledge.
drop policy if exists pilot_services_member_all on public.pilot_services;
create policy pilot_services_select on public.pilot_services for select to authenticated using(pilot_private.has_permission(business_id,'view_services'));
create policy pilot_services_insert on public.pilot_services for insert to authenticated with check(pilot_private.has_permission(business_id,'manage_services'));
create policy pilot_services_update on public.pilot_services for update to authenticated using(pilot_private.has_permission(business_id,'manage_services')) with check(pilot_private.has_permission(business_id,'manage_services'));
create policy pilot_services_delete on public.pilot_services for delete to authenticated using(pilot_private.has_permission(business_id,'manage_services'));
...
create policy pilot_business_knowledge_select on public.pilot_business_knowledge for select to authenticated using(pilot_private.has_permission(business_id,'view_knowledge'));
create policy pilot_business_knowledge_write on public.pilot_business_knowledge for all to authenticated using(pilot_private.has_permission(business_id,'manage_knowledge')) with check(pilot_private.has_permission(business_id,'manage_knowledge'));

-- Customer management/memory.
drop policy if exists pilot_customer_management_member_all on public.pilot_customer_management;
create policy pilot_customer_management_select on public.pilot_customer_management for select to authenticated using(pilot_private.has_permission(business_id,'view_customers'));
create policy pilot_customer_management_write on public.pilot_customer_management for all to authenticated using(pilot_private.has_permission(business_id,'edit_customers')) with check(pilot_private.has_permission(business_id,'edit_customers'));

drop policy if exists pilot_customer_memory_member_all on public.pilot_customer_memory;
create policy pilot_customer_memory_select on public.pilot_customer_memory for select to authenticated using(pilot_private.has_permission(business_id,'view_customers'));
create policy pilot_customer_memory_write on public.pilot_customer_memory for all to authenticated using(pilot_private.has_permission(business_id,'edit_customers')) with check(pilot_private.has_permission(business_id,'edit_customers'));
...
create policy pilot_handoffs_select on public.pilot_handoffs for select to authenticated using(pilot_private.has_permission(business_id,'view_conversations'));
create policy pilot_handoffs_update on public.pilot_handoffs for update to authenticated using(pilot_private.has_permission(business_id,'manage_handoffs') or assigned_user_id=(select auth.uid())) with check(pilot_private.has_permission(business_id,'manage_handoffs') or assigned_user_id=(select auth.uid()));
...
alter table public.pilot_channels add constraint pilot_channels_status_check
...
begin
 ...
 insert into public.pilot_memberships(business_id,user_id,role) values(v_id,v_user,'owner');
 insert into public.pilot_channels(business_id,channel_type,status,metadata) values
...
begin
 ...
 elsif not exists(select 1 from public.pilot_memberships m where m.business_id=v_run.business_id and m.user_id=(select auth.uid()) and m.role in ('owner','admin')) then raise exception 'OWNER_OR_ADMIN_REQUIRED'; end if;
...
create index if not exists pilot_memberships_user_business_role_idx on public.pilot_memberships(user_id,business_id,role);
