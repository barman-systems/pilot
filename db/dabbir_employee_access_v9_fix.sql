...
declare
  ...
  v_old public.dabbir_memberships%rowtype;
  v_new public.dabbir_memberships%rowtype;
  v_email text;
  v_action text;
begin
  if v_actor is null then raise exception 'AUTH_REQUIRED'; end if;
  if p_status not in ('active','suspended','removed') then raise exception 'INVALID_STATUS'; end if;
  select m.* into v_old from public.dabbir_memberships m where m.business_id=p_business_id and m.user_id=p_user_id for update;
  ...
  update public.dabbir_memberships m
  set status=p_status,
  ...
  if p_status='removed' then
    ...
    if v_email is not null then
      update public.dabbir_employee_invitations i
  ...
  if v_action is not null then
    insert into public.dabbir_access_audit(business_id,actor_user_id,target_user_id,action,metadata)
