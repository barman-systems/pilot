...
import { readOwnerBody, ownerBroker } from './_owner-broker-client.js';
...
async function mutationResult(req,res,payload,expected){
 const receipt=payload.payload,id=receipt?.incident_id||receipt?.incident?.id;
 if(receipt?.ok!==true||!UUID.test(id||''))return json(res,502,{ok:false,error:'INCIDENT_RECEIPT_MISSING',retry_safe:false});
 const readback=await ownerBroker(req,'incidents',{incident_id:id});
 const rows=readback.payload?.payload?.incidents;
 const verified=Boolean(readback.payload?.ok&&Array.isArray(rows)&&rows.some(row=>row.id===id&&Object.entries(expected).every(([key,value])=>!value||row[key]===value)));
 return json(res,200,{...payload,readback_verified:verified,retry_safe:false});
}
export default async function handler(req,res){
 ...
 if(req.method==='GET'){
  ...
  if(!Array.isArray(payload.payload?.incidents)||!Array.isArray(payload.payload?.events))return json(res,502,{ok:false,error:'INCIDENT_RESPONSE_INVALID'});
  return json(res,200,payload);
 }
 if(!requireSameOrigin(req))return json(res,403,{ok:false,error:'ORIGIN_REQUIRED'});
 let body;try{body=await readOwnerBody(req,24576)}catch{return json(res,400,{ok:false,error:'INVALID_JSON'})}
 ...
 if(operation==='create'){
  ...
  return mutationResult(req,res,payload,{customer_no:customerNo,summary,priority});
 }
 if(operation==='update'){
  ...
  return mutationResult(req,res,payload,{status:statusValue,priority,assigned_queue:assignedQueue,root_cause:rootCause,resolution});
 }
