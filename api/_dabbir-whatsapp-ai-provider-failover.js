...
export async function recoverWhatsAppAiProviderFailovers({limit=12}={}){
  ...
  for(const row of Array.isArray(rows)?rows:[]){
    ...
    results.push(await failoverWhatsAppAiProvider(token,clean(row?.last_error,240)||'AI_PLANNER_UNAVAILABLE').catch(error=>({handled:false,state:'FAILOVER_ERROR',error:clean(error?.code||error?.message,160)})));
  }
