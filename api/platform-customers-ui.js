const script=String.raw`(()=>{
  ...
  const copy=()=>ar()?{
    nav:'إدارة العملاء',title:'إدارة عملاء DABBIR',desc:'حسابات عملاء المنصة والدعم والتحكم والاسترجاع من مكان واحد.',search:'ابحث برقم DAB أو البريد أو الهاتف أو اسم النشاط',find:'بحث',accounts:'الحسابات',businesses:'الأنشطة',active:'نشط',blocked:'موقوف',suspended:'معلّق في DABBIR',verified:'البريد مؤكد',lastLogin:'آخر دخول',created:'تاريخ التسجيل',phone:'الهاتف',noPhone:'غير مسجل',details:'فتح الحساب',role:'الدور',status:'الحالة',activity:'البيانات التشغيلية',customers:'عملاء النشاط',chats:'المحادثات',messages:'الرسائل',orders:'الطلبات',appointments:'المواعيد',tasks:'المهام',access:'وصول DABBIR',accessDesc:'تعليق الحساب يوقف وصول هذا العميل إلى DABBIR فقط ولا يحظر هوية Supabase أو أي نظام آخر.',suspend:'تعليق الحساب',reactivate:'إعادة تفعيل الحساب',reason:'سبب التعليق',reasonPlaceholder:'مثال: طلب العميل، إساءة استخدام، مشكلة فوترة قيد المراجعة',suspendConfirm:'للتعليق اكتب',suspendedAt:'تم التعليق',accessUpdated:'تم تحديث وصول الحساب.',adminProtected:'لا يمكن تعليق حساب Platform Admin.',reasonRequired:'اكتب سببًا واضحًا للتعليق.',confirmRequired:'عبارة التأكيد غير مطابقة.',recovery:'استرجاع البيانات',recoveryDesc:'اختر وقتًا سابقًا لمساحة العمل. لن يتغير شيء قبل المعاينة والتأكيد.',targetTime:'الوقت المراد الرجوع إليه',preview:'معاينة الاسترجاع',prepare:'إنشاء حالة استرجاع',events:'تغييرات سيتم عكسها',confirmLabel:'للتنفيذ اكتب',apply:'تنفيذ الاسترجاع',restored:'تم تنفيذ الاسترجاع.',empty:'لا توجد نتائج.',loading:'جارٍ التحميل...',failed:'تعذر تحميل لوحة إدارة العملاء.',back:'العودة للحسابات',platformOwner:'مالك المنصة',danger:'هذا الإجراء يعيد بيانات مساحة العمل إلى الوقت المحدد.'
  }:{
    nav:'Customer admin',title:'DABBIR customer administration',desc:'Platform customer accounts, support, access control and recovery in one place.',search:'Search DAB number, email, phone, or business name',find:'Search',accounts:'Accounts',businesses:'Businesses',active:'Active',blocked:'Blocked',suspended:'Suspended in DABBIR',verified:'Email verified',lastLogin:'Last sign-in',created:'Created',phone:'Phone',noPhone:'Not stored',details:'Open account',role:'Role',status:'Status',activity:'Operational data',customers:'Business customers',chats:'Conversations',messages:'Messages',orders:'Orders',appointments:'Appointments',tasks:'Tasks',access:'DABBIR access',accessDesc:'Suspension blocks this customer from DABBIR only. It does not ban the Supabase identity or other systems.',suspend:'Suspend account',reactivate:'Reactivate account',reason:'Suspension reason',reasonPlaceholder:'Example: customer request, abuse, billing review',suspendConfirm:'To suspend, type',suspendedAt:'Suspended',accessUpdated:'Account access updated.',adminProtected:'A Platform Admin account cannot be suspended.',reasonRequired:'Enter a clear suspension reason.',confirmRequired:'Confirmation phrase does not match.',recovery:'Data recovery',recoveryDesc:'Choose an earlier workspace time. Nothing changes before preview and explicit confirmation.',targetTime:'Restore point',preview:'Preview recovery',prepare:'Create recovery case',events:'Changes to reverse',confirmLabel:'To apply, type',apply:'Apply recovery',restored:'Recovery applied.',empty:'No results.',loading:'Loading...',failed:'Customer administration could not load.',back:'Back to accounts',platformOwner:'Platform owner',danger:'This action returns workspace data to the selected time.'
  };
  ...
  function renderDetail(){
    ...
    body.innerHTML='<button class="secondary" id="pcBack">← '+esc(t.back)+'</button><div class="card" style="margin-top:10px"><div class="row space"><div><span class="pcCode">'+esc(a.customer_no||'')+'</span><h2 style="margin:5px 0">'+esc(u.email||'—')+'</h2></div><span class="badge '+statusClass({access:selected.access,...u})+'">'+esc(accountStatus({access:selected.access,...u}))+'</span></div><div class="pcGrid" style="margin-top:10px"><div class="pcAccount"><small>'+esc(t.phone)+'</small><b>'+esc(u.phone||t.noPhone)+'</b></div><div class="pcAccount"><small>'+esc(t.created)+'</small><b>'+esc(fmt(u.created_at))+'</b></div><div class="pcAccount"><small>'+esc(t.lastLogin)+'</small><b>'+esc(fmt(u.last_sign_in_at))+'</b></div></div>'+renderAccess()+'</div><div style="margin-top:12px">'+businesses.map(renderBusiness).join('')+'</div>';
    q('#pcBack').onclick=()=>{selected=null;render()};
    if(q('#pcSuspend'))q('#pcSuspend').onclick=suspendAccount;
    if(q('#pcReactivate'))q('#pcReactivate').onclick=reactivateAccount;
    qa('[data-pc-preview]').forEach(b=>b.onclick=()=>previewRecovery(b.dataset.pcPreview));qa('[data-pc-open]').forEach(b=>b.onclick=()=>openRecovery(b.dataset.pcOpen));qa('[data-pc-apply]').forEach(b=>b.onclick=()=>applyRecovery(b.dataset.pcApply));
  }
  async function suspendAccount(){
    const t=copy(),reason=String(q('#pcSuspendReason')?.value||'').trim(),expected='SUSPEND '+String(selected.account?.customer_no||''),confirmation=String(q('#pcSuspendConfirm')?.value||'').trim();
    if(reason.length<3){if(typeof toast==='function')toast(t.reasonRequired);return}
    if(confirmation!==expected){if(typeof toast==='function')toast(t.confirmRequired);return}
    const {r,j}=await api('/api/platform-customers',{method:'POST',body:JSON.stringify({action:'set_access',user_id:selected.user.id,status:'suspended',reason})});
    if(!r.ok){if(typeof toast==='function')toast(j.error==='PLATFORM_ADMIN_IMMUTABLE'?t.adminProtected:(j.error||t.failed));return}
    if(typeof toast==='function')toast(t.accessUpdated);await openAccount(selected.user.id);
  }
  async function reactivateAccount(){
    const t=copy(),{r,j}=await api('/api/platform-customers',{method:'POST',body:JSON.stringify({action:'set_access',user_id:selected.user.id,status:'active'})});
    if(!r.ok){if(typeof toast==='function')toast(j.error||t.failed);return}
    if(typeof toast==='function')toast(t.accessUpdated);await openAccount(selected.user.id);
  }
...
export default function handler(req,res){
  ...
  res.statusCode=200;res.setHeader('content-type','application/javascript; charset=utf-8');res.setHeader('cache-control','no-store');res.setHeader('x-content-type-options','nosniff');res.setHeader('x-dabbir-platform-customer-admin-ui','v2');return res.end(script);
}
