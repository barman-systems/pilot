...
import { runQwen35Benchmark } from './_dabbir-qwen35-benchmark.js';
...
export default async function handler(req, res) {
  ...
  try {
    const result = await runQwen35Benchmark({ scenario: String(req.body?.scenario || 'critical') });
    return json(res, result.ok ? 200 : 502, {
      ...result,
      service: 'dabbir-qwen35-benchmark',
      access_boundary: 'VERCEL_PROTECTED_PREVIEW_PLUS_SAME_ORIGIN_SYNTHETIC_SCOPE',
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    const code = String(error?.message || 'QWEN35_BENCHMARK_FAILED');
    const safe = /^(?:QWEN35_BENCHMARK_[A-Z_]+)$/.test(code) ? code : 'QWEN35_BENCHMARK_FAILED';
