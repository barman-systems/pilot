const script=String.raw`(()=>{
  ...
  let loadedBusiness=null,proof=null,proofLoading=false,asking=false;
  ...
  function render(){
    ...
    card.innerHTML='<div class="dcHead"><div class="dcIdentity"><img class="dcLogo" src="/api/dabbir-approved-icon" alt=""><div><h2>'+esc(t.title)+'</h2><p>'+esc(t.desc)+'</p></div></div><span class="dcMode">'+esc(t.mode)+'</span></div>'+
      ...
      '<form class="dcAsk" id="dcAskForm"><input class="dcInput" id="dcAskInput" maxlength="800" autocomplete="off" enterkeyhint="send" placeholder="'+esc(t.placeholder)+'"><button id="dcAskButton" type="submit" '+(asking?'disabled':'')+'>'+esc(asking?t.loading:t.ask)+'</button></form>'+
      ...
      '<div class="dcAnswer '+(priorAnswer?'show':'')+'" id="dcAnswer"><div class="dcAnswerText" id="dcAnswerText">'+esc(priorAnswer)+'</div><div class="dcAnswerMeta" id="dcAnswerMeta" data-source="'+esc(priorMeta)+'"><b>'+esc(priorMeta==='DETERMINISTIC_VERIFIED_FALLBACK'?t.fallback:t.verified)+'</b><span>Asia/Dubai</span></div></div>';
  ...
  async function ask(message){
    ...
    try{
      ...
      asking=false;render();showAnswer(body.answer,body.answer_source);
    }catch(error){asking=false;render();showAnswer(copy().error,'');try{if(typeof toast==='function')toast(copy().error)}catch{}}
  }
  ...
  if(typeof renderAll==='function'){
    const base=renderAll;renderAll=function(){const result=base.apply(this,arguments);if(window.current==='dashboard'||typeof current==='undefined')setTimeout(()=>{render();loadProof(false)},0);return result};
  }
