...
export default async function handler(req,res){
  ...
  try{
    ...
    const status=Number(error?.code||500);const safe=[400,401,403,404,413,429,503,504].includes(status)?status:502;console.error('dabbir_billing_portal_failed',{status:safe,error:String(error?.message||'PORTAL_FAILED').slice(0,120)});return json(res,safe,{ok:false,error:String(error?.message||'PORTAL_FAILED').slice(0,120),livemode:false});
  }
}