...
import { getBillingAccount, requestOrigin, requireBillingOwner, stripeRequest } from '../_billing-core.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    const session=await stripeRequest('/billing_portal/sessions',{method:'POST',params:{customer:account.stripe_customer_id,return_url:`${requestOrigin(req)}/?billing=portal_return`}});
    if(!session?.url)throw Object.assign(new Error('PORTAL_URL_MISSING'),{code:502});return json(res,200,{ok:true,url:session.url,livemode:false});
  }catch(error){
