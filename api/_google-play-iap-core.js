...
function supabaseUrl() {
  ...
  if (!/^https:\/\//i.test(value)) throw playError('GOOGLE_PLAY_STORAGE_NOT_CONFIGURED', 503);
  return value;
}
...
async function googleAccessToken() {
  ...
  if (!response.ok || !token || !Number.isFinite(expiresIn) || expiresIn <= 0) throw playError('GOOGLE_PLAY_OAUTH_FAILED', 503);
...
function parseSubscription(payload, purchaseToken, userId) {
  ...
  if (!UUID_RE.test(userId) || externalId.toLowerCase() !== userId.toLowerCase()) throw playError('GOOGLE_PLAY_ACCOUNT_MISMATCH', 403);
  ...
  return {
    ...
    status,
    acknowledgement_state: String(payload?.acknowledgementState || 'ACKNOWLEDGEMENT_STATE_UNSPECIFIED').slice(0, 80),
    auto_renew_enabled: Boolean(item?.autoRenewingPlan?.autoRenewEnabled),
...
export async function verifyGoogleSubscription(purchaseTokenValue, userId) {
  ...
  const payload = await googlePublisher(`applications/${encodeURIComponent(packageName())}/purchases/subscriptionsv2/tokens/${encodeURIComponent(purchaseToken)}`);
...
export async function persistGoogleEntitlement(entitlement) {
  ...
  const response = await fetch(`${supabaseUrl()}/rest/v1/dabbir_google_entitlements?on_conflict=user_id`, {
    method: 'POST',
...
export async function loadGoogleEntitlement(userId, { refresh = true } = {}) {
  if (!UUID_RE.test(String(userId || ''))) throw playError('GOOGLE_PLAY_USER_INVALID', 400);
  const key = serviceRoleKey();
  const response = await fetch(`${supabaseUrl()}/rest/v1/dabbir_google_entitlements?select=*&user_id=eq.${encodeURIComponent(userId)}&limit=1`, {
