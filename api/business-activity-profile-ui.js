...
const profiles={
  ...
  services:{
    hideDelivery:true,
    fields:['customer_requirements','activity_operations'],
    ar:{name:'الخدمات',title:'تعليمات خاصة',desc:'دبّر يقرأ الخدمات والأسعار ومدة كل خدمة تلقائيًا من قسم الخدمات. أضف هنا فقط ما لا يستطيع معرفته من البيانات المسجلة.',labels:{customer_requirements:'ما يحتاجه العميل قبل الخدمة',activity_operations:'استثناءات أو تعليمات التنفيذ'},placeholders:{customer_requirements:'اختياري: بيانات أو تجهيزات خاصة مطلوبة من العميل',activity_operations:'اختياري: استثناءات أو شروط تنفيذ لا تظهر في بيانات الخدمة'}},
    en:{name:'Services',title:'Special instructions',desc:'DABBIR automatically reads services, prices, and service duration from the Services section. Add only details it cannot know from the saved service data.',labels:{customer_requirements:'Customer preparation',activity_operations:'Execution exceptions or instructions'},placeholders:{customer_requirements:'Optional: special information or preparation required from the customer',activity_operations:'Optional: execution exceptions or conditions not represented in service data'}}
  },
  real_estate:{
    hideDelivery:true,
