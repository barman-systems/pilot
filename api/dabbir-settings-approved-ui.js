...
@media(max-width:700px){
  ...
  body.dabbir-settings-approved .dk-hours-legacy{border-radius:12px!important;font-size:10px!important}
...
`;
  ...
  function installHeaderLogo(){
    ...
    if(!img){
      ...
      img.className='dsa-header-logo';
      img.src='/dabbir-app-icon.png';
      img.alt='DABBIR';
      img.decoding='async';
      top.append(img);
    }
  ...
  function toggleLanguage(){
    ...
    try{
      if(typeof setLanguage==='function')setLanguage(ar()?'en':'ar');
      else if(typeof applyLang==='function')applyLang(ar()?'en':'ar');
    }catch{}
  }
  ...
  function businessOpenNow(){
    ...
    if(!hit)return false;
    const start=parseClock(hit[1]),end=parseClock(hit[2]);
  ...
  function ensureToolbar(){
    ...
    const card=screen?.querySelector('.dabbir-knowledge-card');
    if(!screen||!card)return;
    let toolbar=screen.querySelector('.dsa-settings-toolbar');
    if(!toolbar){
      toolbar=document.createElement('div');
      toolbar.className='dsa-settings-toolbar';
      ...
      toolbar.querySelector('.dsa-language-control')?.addEventListener('click',()=>{toggleLanguage();setTimeout(sync,60)});
      card.before(toolbar);
    }
  ...
  document.addEventListener('change',event=>{
    if(event.target?.matches?.('[id^="dk-day-"],[id^="dk-start-"],[id^="dk-end-"]'))setTimeout(ensureToolbar,0);
  },true);
  document.addEventListener('click',event=>{
    if(event.target?.closest?.('[data-screen="settings"],#menuBtn,.navBtn,.bottomNav button,.bottomNav a'))setTimeout(sync,0);
  },true);

  const observer=new MutationObserver(()=>sync());
  observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','lang','dir']});
  setInterval(()=>{if(active())ensureToolbar()},60000);
  sync();
  setTimeout(sync,100);
  setTimeout(sync,500);
})();`;
