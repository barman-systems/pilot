...
export function understandTurnV3({context,proposal,previousState,now=new Date()}){
  ...
  const wideMode=activityWideSingleMode(context),semanticIntent=String(proposal?.intent||'').toUpperCase();
  if(wideMode&&['BOOKING','SERVICE_DISCOVERY','PRICING','RESCHEDULE_BOOKING'].includes(semanticIntent)){const f=fact('delivery_mode',wideMode,'DATABASE_FACT','ACTIVITY_SINGLE_MODE');putFact(factMap,f);turnVerified.push(f);}

  let selectedService=null;
  if(proposal?.serviceName){
    const candidate=serviceByName(context,proposal.serviceName);
    if(candidate&&proposal?.serviceVerified===true){
      selectedService=candidate;const f=fact('service',candidate.id,'CUSTOMER_CONFIRMED','V3_SCOPED_CATALOG_VERIFIED_SELECTION',{surface:proposal.serviceSurface||proposal.serviceName,confidence:1});putFact(factMap,f);turnVerified.push(f);
      if(Number.isFinite(Number(candidate.price))){const p=fact('price',Number(candidate.price),'DATABASE_FACT','SCOPED_CATALOG_PRICE');putFact(factMap,p);turnVerified.push(p);}
    }else if(candidate){
      turnTentative.push(tentative('service',candidate.id,proposal.serviceSurface||proposal.serviceName,'SCOPED_CATALOG_CANDIDATE_NEEDS_CONFIRMATION',{source:'V3_SEMANTIC_PROPOSAL',confidence:Number(proposal?.confidence)||0}));
    }
  }
  if(!selectedService){const serviceFact=factMap.get('service');if(serviceFact)selectedService=serviceById(context,serviceFact.value);}
  let contract=selectedService?contractFor(context,selectedService.id):null;

  if(role==='CONFIRMATION'&&previousState?.pending_question?.purpose==='CONFIRM_TENTATIVE_SERVICE'){
    const priorService=previousTentatives.find(x=>x.field==='service'&&x.candidate_value),row=priorService?serviceById(context,priorService.candidate_value):null;
  ...
  for(const item of arr(proposal?.entities).slice(0,16)){
    const field=clean(item?.entity,80),surface=clean(item?.evidence,300),value=item?.value,confidence=Number(item?.confidence)||0;if(!field||!evidenceOk(raw,surface))continue;
    const def=contract?.entity_definitions?.[field]||unionDefinition(context,field);
    if(field==='date'&&validDate(value)){const f=fact(field,value,'CUSTOMER_STATED','V3_SEMANTIC_DATE',{surface,confidence});putFact(factMap,f);turnVerified.push(f);continue;}
    if(field==='time'&&validTime(value)){const f=fact(field,value,'CUSTOMER_STATED','V3_SEMANTIC_TIME',{surface,confidence});putFact(factMap,f);turnVerified.push(f);continue;}
    if(field==='vehicle'){const allowed=def?.type==='ENUM'&&arr(def.values).includes(value);turnTentative.push(tentative(field,value,surface,allowed?'ALLOWED_VALUE_NEEDS_GROUNDING':'SEMANTIC_VALUE_NEEDS_MAPPING',{confidence}));continue;}
    ...
    if(field==='delivery_mode'){const allowed=def?.type==='ENUM'&&(!arr(def.values).length||arr(def.values).includes(value));if((wideMode&&value===wideMode)||(contract&&arr(contract.delivery_modes).length===1&&value===contract.delivery_modes[0])){const f=fact(field,value,'DATABASE_FACT','SINGLE_MODE_CONFIRMED',{surface,confidence:1});putFact(factMap,f);turnVerified.push(f);}else if(allowed)turnTentative.push(tentative(field,value,surface,'ALLOWED_VALUE_NEEDS_GROUNDING',{confidence}));continue;}
    if(def?.type==='TEXT')turnTentative.push(tentative(field,value??surface,surface,'TEXT_NEEDS_DIALOGUE_CONFIRMATION',{confidence}));
  }

  if(role==='CONFIRMATION'&&previousState?.pending_question?.purpose==='CONFIRM_TENTATIVE_VEHICLE'){const priorVehicle=previousTentatives.find(x=>x.field==='vehicle'&&x.candidate_value);if(priorVehicle){const f=fact('vehicle',priorVehicle.candidate_value,'CUSTOMER_CONFIRMED','TENTATIVE_CONFIRMED_BY_CUSTOMER',{surface:raw,confidence:1});putFact(factMap,f);turnVerified.push(f);invalidations.push({field:'vehicle',reason:'TENTATIVE_CONFIRMED'});}}
  ...
  if(pendingField&&role==='ANSWER_TO_PENDING_QUESTION'&&!turnVerified.some(f=>f.field===pendingField)&&!turnTentative.some(f=>f.field===pendingField)&&raw){if(!['SIDE_QUESTION','SOCIAL','TOPIC_SWITCH','CANCELLATION'].includes(role)&&!['service','slot','location'].includes(pendingField))turnTentative.push(tentative(pendingField,null,raw,'SEMANTIC_SURFACE_UNMAPPED',{source:'CURRENT_TURN_SURFACE',confidence:Number(proposal?.confidence)||0}));}

  const parts=localParts(safeAt,context?.business?.timezone||'Asia/Dubai'),date=factMap.get('date'),time=factMap.get('time');
  if(parts&&date?.value===parts.date&&time?.value===parts.time&&turnVerified.some(f=>f.field==='date'||f.field==='time')){const f=fact('immediacy','NOW','CUSTOMER_STATED','MESSAGE_RECEIPT_TIME');putFact(factMap,f);turnVerified.push(f);}
  const serviceQuestion=proposal?.serviceQuestion?.field||null;if(serviceQuestion)sideQuestions.push({type:serviceQuestion,evidence:proposal.serviceQuestion?.evidence||null});else if(String(proposal?.action||'').toUpperCase()==='PRICING')sideQuestions.push({type:'price',evidence:null});
  ...
  const serviceSelectedThisTurn=turnVerified.some(f=>f.field==='service');let goal=previousState?.goal||'UNKNOWN';
  if(semanticIntent==='BOOKING')goal='BOOK_SERVICE';else if(semanticIntent==='RESCHEDULE_BOOKING')goal='RESCHEDULE_BOOKING';else if(semanticIntent==='CANCEL_BOOKING')goal='CANCEL_BOOKING';else if(semanticIntent==='SERVICE_DISCOVERY'&&!['BOOK_SERVICE','RESCHEDULE_BOOKING'].includes(goal))goal='DISCOVER_SERVICE';else if(semanticIntent==='PRICING'&&!['BOOK_SERVICE','RESCHEDULE_BOOKING'].includes(goal))goal='PRICE_SERVICE';else if(semanticIntent==='SUPPORT'&&goal==='UNKNOWN')goal='SUPPORT';
  const currentTentativeFields=new Set(turnTentative.map(x=>x.field)),confirmedTurnFields=new Set(turnVerified.map(x=>x.field));
