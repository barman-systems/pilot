...
const liveScript=String.raw`(()=>{
  ...
  function httpError(response,body,fallback){const error=new Error(body?.error||fallback);error.status=Number(response?.status||0);return error}
  ...
  async function connectionState(id,force=false){const now=Date.now();if(!force&&connectionBlockedUntil>now){const error=new Error(connectionBlockedError||'AUTH_REQUIRED');error.status=401;throw error}if(!force&&lastConnectionState&&lastBusiness===id&&now-lastConnectionCheckAt<PASSIVE_CACHE_MS)return lastConnectionState;const response=await fetch('/api/calendar-connections?business_id='+encodeURIComponent(id),{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json'}});const body=await response.json().catch(()=>null);if(!response.ok||!body?.ok){const error=httpError(response,body,'CALENDAR_CONNECTIONS_FAILED');if(error.status===401||error.status===403){connectionBlockedUntil=Date.now()+AUTH_BACKOFF_MS;connectionBlockedError=String(error.message||'AUTH_REQUIRED')}throw error}lastConnectionState=body;lastConnectionCheckAt=Date.now();connectionBlockedUntil=0;connectionBlockedError='';return body}
  async function loadBusy(id,force=false){const now=Date.now();if(!force&&lastBusiness===id&&lastBusyLoadAt&&now-lastBusyLoadAt<PASSIVE_CACHE_MS){renderBusy();removeCancelledFromActiveCalendar();return}const response=await fetch('/api/calendar-sync?business_id='+encodeURIComponent(id),{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json'}});const body=await response.json().catch(()=>null);if(!response.ok||!body?.ok)throw httpError(response,body,'CALENDAR_BUSY_FAILED');lastBusy=Array.isArray(body.busy_blocks)?body.busy_blocks:[];lastBusyLoadAt=Date.now();renderBusy();removeCancelledFromActiveCalendar()}
  async function runSync(force=false){const id=businessId();if(!id)return;ensureUi();if(id!==lastBusiness)resetPassiveState(id);try{const connections=await connectionState(id,force),active=(connections.connections||[]).filter(c=>c.status==='active'&&c.sync_enabled!==false);if(!active.length){lastBusy=[];lastBusyLoadAt=Date.now();renderBusy();removeCancelledFromActiveCalendar();return}const due=force||Date.now()-lastSyncAt>5*60*1000;if(due){busy=true;ensureUi();const response=await fetch('/api/calendar-sync',{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json',accept:'application/json'},body:JSON.stringify({business_id:id})});const body=await response.json().catch(()=>null);if(!response.ok||!body?.ok)throw httpError(response,body,'CALENDAR_SYNC_FAILED');lastSyncAt=Date.now();lastBusyLoadAt=0;try{window.__dabbirActivityProfile?.refresh?.()}catch{}}await loadBusy(id,due||force);removeCancelledFromActiveCalendar();if(force)try{toast(ar()?'تمت مزامنة التقويم':'Calendar synced')}catch{}}catch(error){const status=Number(error?.status||0);if(force||status>=500||status===0)console.error('dabbir_calendar_live_ui_failed',String(error?.message||error).slice(0,120));if(force)try{toast(ar()?'تعذرت مزامنة التقويم':'Calendar sync failed')}catch{}}finally{busy=false;ensureUi();removeCancelledFromActiveCalendar()}}
...
export default async function handler(req,res){
  ...
  const activityCaptured=captureResponse(),managementCaptured=captureResponse(),internalCalendarCaptured=captureResponse(),salonCaptured=captureResponse(),clinicCaptured=captureResponse(),businessActivityCaptured=captureResponse();
