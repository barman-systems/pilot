...
export default async function handler(req,res){
  ...
  try{
    if(req.method==='GET'){
      ...
      return json(res,200,{ok:true,business_id:businessId,can_manage:canManage(membership),services:(services||[]).filter(row=>!isDeleted(row)).map(normalized)});
    }
    if(req.method==='POST'){
      ...
      if(action==='delete_service'){
        ...
        const currentRows=await rest(ctx.token,`dabbir_services?id=eq.${serviceId}&business_id=eq.${businessId}&select=id,name,price_aed,duration_minutes,active,metadata&limit=1`,{},'SERVICE_LOOKUP_FAILED');
        ...
        const rows=await rest(ctx.token,`dabbir_services?id=eq.${serviceId}&business_id=eq.${businessId}&select=id,name,price_aed,duration_minutes,active,metadata`,{
      ...
      const name=clean(body.name,160),duration=Math.trunc(finite(body.duration_minutes)),price=finite(body.price_aed);
      ...
      if(action==='create_service'){
        const rows=await rest(ctx.token,'dabbir_services?select=id,name,price_aed,duration_minutes,active,metadata',{method:'POST',headers:{prefer:'return=representation'},body:JSON.stringify({business_id:businessId,name,price_aed:Number(price.toFixed(2)),duration_minutes:duration,active:true,metadata:{source:'owner_service_catalog'}})},'SERVICE_CREATE_FAILED');
