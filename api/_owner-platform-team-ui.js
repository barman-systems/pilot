...
export function ownerPlatformTeamClient({api,t,esc,date,number,can,root,openAction,writeMessage,describeError}){
  ...
  function render(){
    host.innerHTML='<div class="row" data-ui-part="owner-team-detail-1">'+(can('manage_employees','team.invite')?button('invite','',t('دعوة مساعد','Invite teammate')):'')+'</div><div class="stack">'+state.staff.map(row=>{
