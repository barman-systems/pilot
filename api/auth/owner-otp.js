...
const validLogin=login=>login===ROOT_USERNAME||EMAIL_RE.test(login);
const secureCookie=(name,value,maxAge)=>`${name}=${encodeURIComponent(value)}; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age=${Math.max(0,maxAge)}`;
const challengeCookie=(value,maxAge=600)=>secureCookie(CHALLENGE_COOKIE,value,maxAge);
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='verify'){
      ...
      return json(res,200,{ok:true,authenticated:true,authority_role:payload.authority_role,root_owner:payload.root_owner===true,permissions:Array.isArray(payload.permissions)?payload.permissions:[],expires_in:maxAge});
    }
    return json(res,400,{ok:false,error:'INVALID_OTP_ACTION'});
  }catch(error){
    const code=Number(error?.code||500);
    return json(res,code===400||code===413?code:503,{ok:false,error:error?.message==='PAYLOAD_TOO_LARGE'?'PAYLOAD_TOO_LARGE':error?.message==='INVALID_JSON'?'INVALID_JSON':'OWNER_AUTH_UNAVAILABLE'});
  }
}
