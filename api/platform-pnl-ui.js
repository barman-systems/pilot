export const platformPnlScript=String.raw`(()=>{
  ...
  function ensure(){const h=host();if(!h)return null;let el=document.querySelector('#dabbirPnlPanel');if(!el){el=document.createElement('section');el.id='dabbirPnlPanel';el.className='dabbirPnl panel';if(h.anchor)h.parent.insertBefore(el,h.anchor);else h.parent.appendChild(el)}return el}
  ...
  async function capability(){
    if(capabilityChecked)return capabilityAllowed;
    if(capabilityPromise)return capabilityPromise;
    capabilityPromise=(async()=>{try{
      const response=await fetch('/api/owner-finance?action=capability',{credentials:'same-origin',cache:'no-store'});
      const payload=await response.json().catch(()=>null);
      capabilityAllowed=Boolean(response.ok&&payload?.ok&&payload?.allowed);capabilityChecked=true;denied=!capabilityAllowed;return capabilityAllowed;
    }catch{capabilityAllowed=false;capabilityChecked=true;denied=true;return false}
    finally{capabilityPromise=null;render()}})();
    return capabilityPromise;
  }
  async function load(){
    if(loading)return;
    const allowed=await capability();if(!allowed){denied=true;data=null;error=false;render();return}
    loading=true;error=false;render();try{
      const response=await fetch('/api/owner-finance?month='+encodeURIComponent(selectedMonth),{credentials:'same-origin',cache:'no-store'});
      if(response.status===401||response.status===403){denied=true;capabilityAllowed=false;capabilityChecked=true;data=null;error=false;return}
  ...
  const observer=new MutationObserver(()=>{if(ensure()&&!data&&!loading&&!denied&&!capabilityPromise)load()});observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['lang']});
  window.__dabbirUiLifecycle?.on?.('afterNavigate','platform-pnl-v2',()=>{ensure();if(!data&&!loading&&!denied&&!capabilityPromise)load()});
  window.__dabbirUiLifecycle?.on?.('afterLanguage','platform-pnl-lang-v2',()=>render());
  setTimeout(()=>{if(ensure())load()},0);
})();`;
