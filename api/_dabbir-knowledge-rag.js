...
export async function embedKnowledgeText(text,{env=process.env,fetchImpl=fetch,providerHealthStore,providerTrace=[],timeoutMs=12000}={}){
  ...
  const authorityFetch=createAiProviderAuthorityFetch({env,fetchImpl,healthStore:providerHealthStore,trace:providerTrace});
  const response=await authorityFetch(config.endpoint,{method:'POST',headers:{'content-type':'application/json','x-goog-api-key':config.key},
...
export async function retrieveDabbirKnowledge({businessId,query,rpc,env=process.env,fetchImpl=fetch,providerHealthStore,limit=5}={}){
...
export async function indexApprovedKnowledge({rpc,env=process.env,fetchImpl=fetch,providerHealthStore,limit=12}={}){
