...
export async function planAutonomousRun({token,userId,businessId,goal,language,validateWrite}){
  ...
  const finalized=budget?await finalizeAiBudget({businessId,operationKey:budgetOperationKey,outcome:'VERIFIED_SUCCESS',failureClass:null,actualCostUsd:cost.total_cost_usd,metadata:{provider:selected.name,model:selected.modelId,cost_tier:selected.costTier,budget_pressure:budget.budget_pressure?.band||null,attempts,generation:cost,state:cost.total_cost_usd==null?'RESERVATION_RETAINED':'ACTUAL_COST_VERIFIED'}}).then(()=>true).catch(()=>false):true;
  const budgetEvidence=budget?{hard_limit_aed:HARD_MONTHLY_AI_BUDGET_AED,reservation_aed:budget.reservation_aed,reservation_microusd:budget.reserve_microusd,gateway_spend_usd_before:budget.external_spend_usd,budget_pressure:budget.budget_pressure,generation:cost,ledger_finalized:finalized}:{hard_limit_aed:HARD_MONTHLY_AI_BUDGET_AED,paid_budget_claimed:false,route:'FREE_DIRECT_PROVIDER',generation:cost,ledger_finalized:true};
