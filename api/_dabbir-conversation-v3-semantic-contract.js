...
const ENTITIES=['vehicle','date','time','time_window','immediacy','delivery_mode','property_details'];
...
entities contain only facts stated or corrected in the CURRENT message. surface must be an exact current-message quote. vehicle value may be station or saloon when the meaning is clear; otherwise preserve the customer's wording as value. Dates are YYYY-MM-DD and exact times are HH:mm using reference_time and business timezone. Never invent a GPS location: typed location language is not a verified location.
When the customer explicitly means immediately/currently now (for example Gulf Arabic equivalents of “now” or English “now/right now”), emit immediacy with value NOW. Do not invent a clock time for immediacy: the application derives the exact business-local receipt date/time from reference_time. If the current message contains a conflicting explicit future date, exact time or time window, do not emit immediacy=NOW.
For non-exact time language such as الصبح, أول الصباح, morning, afternoon, evening or night, emit time_window instead of inventing an exact time. time_window value must be one of EARLY_MORNING, MORNING, AFTERNOON, EVENING, NIGHT. A time_window never satisfies an exact appointment time by itself.
...
When a customer corrects an exact/now time to a time_window, mark the new entity correction=true and invalidate time, slot and immediacy. When a customer corrects to an exact time, invalidate time_window, slot and immediacy. When the customer explicitly changes the request to NOW, emit immediacy=NOW with correction=true and invalidate date, time_window and slot; the application will replace date/time from the current receipt time. When the date changes, invalidate slot and immediacy. Do not preserve NOW after a conflicting temporal correction.
...
export function v3SemanticContractViolation(raw){
  ...
  if(!Array.isArray(x.invalidated_fields)||x.invalidated_fields.length>10||!x.invalidated_fields.every(f=>INVALIDATIONS.includes(f)))return 'INVALIDATIONS';
  return null;
}
