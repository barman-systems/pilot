...
export default async function handler(req,res){
  ...
  try{
    ...
    // #898 routed the internal Web customer test through the service-oriented
    // WhatsApp semantic runtime before that runtime carried store product and
    // inventory facts. The exact Production customer journey then returned
    // HTTP 200 with no AI message and could later create an unnecessary handoff.
    // Keep Web on the previously verified grounded path until channel parity has
    // store read authority plus a deterministic Production promotion proof.
