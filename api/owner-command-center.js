...
import { renderOwnerProductTruthPanel, ownerProductTruthClient } from './_owner-product-truth-partial.js';
...
<section id="support" class="screen" hidden>${subnav('support',[['cases','القضايا','Cases'],['incidents','الحوادث','Incidents'],['feedback','الملاحظات','Feedback']])}<div class="row" data-ui-part="owner-console-detail-1"><button id="newSupportCase" class="primary">${t('قضية دعم جديدة','New support case')}</button><button id="newIncident">${t('تسجيل حادث','Record incident')}</button></div>${status('support')}<div id="supportContent"></div></section>
...
</main><script>(${ownerDashboardClient.toString()})(${scriptJson(identity)},${scriptJson(lang)},${ownerPlatformTeamClient.toString()},${ownerProductTruthClient.toString()});</script></body></html>`;
}
...
export function ownerDashboardClient(identity,lang,mountTeam,mountProductTruth){
  ...
  async function openCustomer(index){
    ...
    await load('customers',dataUrl('customer360',{user_id:selected.user_id}),p=>{
      ...
      $('customerDetail').innerHTML='<h2>'+esc(p.customer_no)+'</h2>'+details([[t('البريد','Email'),p.account?.email],[t('آخر دخول','Last sign in'),date(p.account?.last_login)]])+'<h3 data-ui-part="owner-console-detail-2">'+t('الأنشطة','Businesses')+'</h3>'+list([t('النشاط','Business'),t('الاشتراك','Subscription'),'WhatsApp',t('الإجراء','Action')],businesses.map(b=>[esc(b.name),esc(b.plan?.status||t('غير مشترك','Not subscribed')),esc(b.whatsapp?.state||'—'),button('data-business',b.id,t('فتح العمليات','Open operations'))]))+'<div class="row" data-ui-part="owner-console-detail-3"><a class="button" href="#support/cases">'+t('دعم العميل','Customer support')+'</a></div>';
  ...
  function route(){
    ...
    if(primary==='home'){void loadOverview();void productTruth.refresh()}else if(primary==='customers')void searchCustomers();else if(primary==='operations')void loadOperations();else if(primary==='support')void loadSupport();else if(primary==='ceo')void loadCeo();else void loadSystem();
  }
  ...
  const productTruth=mountProductTruth({api,t,esc,date,number,root,list,stat});
