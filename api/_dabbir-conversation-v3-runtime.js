...
function confirmedFactsFromEntities(entities){return Object.fromEntries(Object.entries(entities||{}).filter(([,f])=>f?.status==='active'&&f.value!=null).map(([field,f])=>[field,{value:f.value,source:f.source,confidence:f.confidence}]));}
...
export const _v3RuntimeTest={authorityProjection,authorityAction,metricsFor,preserveUnmappedService,confirmedFactsFromEntities};
