...
function slotsText(slots,lang){const lines=arr(slots).slice(0,3).map((s,i)=>`${i+1}) ${fmtWhen(s.starts_at,s.timezone,lang)}${s.worker_name?(lang==='ar'?` مع ${s.worker_name}`:` with ${s.worker_name}`):''}`);if(!lines.length)return lang==='ar'?'لا يوجد وقت متاح قريب من طلبك. أعطني وقتًا آخر يناسبك.':'No nearby slot is available. Send me another time that works for you.';return lang==='ar'?`المتاح:\n${lines.join('\n')}\nاختر الوقت المناسب.`:`Available:\n${lines.join('\n')}\nChoose the time that works for you.`;}
export function bookingText(result,lang){
  ...
  if(result?.confirmation_gate==='deposit'&&result?.status!=='confirmed'){const amount=Number(result?.deposit_required_amount||0),currency=clean(result?.deposit_currency_code||result?.currency_code,8);return lang==='ar'?`تم تسجيل موعدك ✅ ${service?`${service} — `:''}${when}${worker?` مع ${worker}`:''}. يحتاج عربون ${amount} ${currency} للتأكيد.`:`Your appointment is recorded ✅ ${service?`${service} — `:''}${when}${worker?` with ${worker}`:''}. A ${amount} ${currency} deposit is required to confirm it.`;}
...
async function reserveReply(claim,context,body,purpose){const text=clean(body,4000),key=claim.semantic_version?`wa-understanding:${claim.batch_id}:${clean(purpose,24)}`:`wa-ai:${claim.batch_id}:attempt:${claim.attempt_count}:${clean(purpose,24)}`;const row=one(claim.semantic_version?await serviceRpc('dabbir_semantic_reserve_outbound_v2',{p_batch_id:claim.batch_id,p_lock_token:claim.lock_token,p_version:claim.semantic_version,p_key:key,p_hash:hash(text),p_body:text}):await serviceRpc('dabbir_whatsapp_ai_reserve_outbound',{p_business_id:context.business.id,p_conversation_id:context.conversation.id,p_idempotency_key:key,p_payload_hash:hash(text),p_body:text}));if(!row?.reservation_id)throw Object.assign(new Error('AI_OUTBOUND_RESERVATION_UNVERIFIED'),{code:'AI_OUTBOUND_RESERVATION_UNVERIFIED'});return row;}
async function deliver(claim,context,body,purpose='reply',transport=null){
  ...
  try{const sent=transport?await transport(connection,reservation.recipient_handle):await sendMetaText({connection,businessId:context.business.id,recipient:reservation.recipient_handle,body});try{return {...await finalizeOutboundReply({reservationId:reservation.reservation_id,providerMessageId:sent.providerMessageId}),providerMessageId:sent.providerMessageId}}catch(error){await markOutboundResult(reservation.reservation_id,'AMBIGUOUS','WHATSAPP_OUTBOUND_FINALIZE_UNCERTAIN');error.ambiguous=true;throw error}}
  catch(error){if(error?.ambiguous===true)await markOutboundResult(reservation.reservation_id,'AMBIGUOUS',clean(error?.code||error?.message,160));else await markOutboundResult(reservation.reservation_id,'FAILED',Number(error?.providerStatus)===429?'META_HTTP_429':clean(error?.code||error?.message,160));throw error;}
}
...
async function processClaim(claim){
  const context=await serviceRpc('dabbir_whatsapp_ai_context',{p_batch_id:claim.batch_id,p_lock_token:claim.lock_token});if(!context?.business?.id||!context?.conversation?.id)throw Object.assign(new Error('AI_CONTEXT_UNVERIFIED'),{code:'AI_CONTEXT_UNVERIFIED'});
  const routingLoad=await serviceRpc('dabbir_semantic_load_v2',{p_batch_id:claim.batch_id,p_lock_token:claim.lock_token});const mode=routingLoad?.cognitive_policy?.mode||'shadow';
  if(mode==='canary'||mode==='active'){
    return runConversationV3Runtime({claim,context,rpc:serviceRpc,deliver,finish,handoff,bookingText,slotsText,preloadedLoad:routingLoad});
  }
  ...
  return runUnderstandingTurn({claim,context,rpc:shadow.rpc,deliver,finish,handoff,bookingText,slotsText,resolveProduct:resolveCatalogService,cognitiveMode:'policy',
    deliverMenu:async(guarded,c,lang)=>{
      ...
      if(flow?.id){try{return await deliver(guarded,c,lang==='ar'?'أكمل طلب الحجز في نموذج واحد.':'Complete your booking request in one form.','booking-flow',(conn,recipient)=>sendMetaBookingFlow({context:c,connection:conn,recipient,lang,flow}));}catch(error){if(error?.ambiguous===true||Number(error?.providerStatus)===429)throw error;const safeFallback=error?.flowFallbackSafe===true||(error?.definitive===true&&Number(error?.providerStatus)>=400&&Number(error?.providerStatus)<500);if(!safeFallback)throw error;}}
      const menu=await catalogMenuForContext({context:c,connection,allowSync:false});if(!menu?.items?.length)return null;try{return await deliver(guarded,c,lang==='ar'?'اختر الخدمة التي تريدها من الكتالوج.':'Choose the service you want from the catalog.','catalog-products',(conn,recipient)=>sendMetaCatalogProducts({connection:conn,businessId:c.business.id,recipient,catalogId:menu.catalogId,items:menu.items,lang}));}catch(error){if(error?.ambiguous!==true&&error?.definitive===true&&Number(error?.providerStatus)!==429)return null;throw error;}
    },
    planner:async(c,safeContext)=>{
      const message=latestText(c);const retrieved=await retrieveDabbirKnowledge({businessId:c.business.id,query:message,rpc:serviceRpc,env:process.env,fetchImpl:fetch,limit:5});
...
async function requireHumanForFailure(claim,code,reason){try{const context=await serviceRpc('dabbir_whatsapp_ai_context',{p_batch_id:claim.batch_id,p_lock_token:claim.lock_token});await handoff(context,reason,code,'SUPPORT')}catch{}await finish(claim,'HUMAN_REQUIRED',code).catch(()=>null);return {state:'HUMAN_REQUIRED',error:code};}
async function handleFailure(claim,error){const code=clean(error?.code||error?.message||'AI_BATCH_FAILED',240);if(code==='SEMANTIC_SUPERSEDED'||code==='SEMANTIC_VERSION_CONFLICT'){await finish(claim,'CANCELLED',code).catch(()=>null);return {state:'CANCELLED',error:code};}if(error?.ambiguous===true){return requireHumanForFailure(claim,`AMBIGUOUS_OUTBOUND:${code}`,'Ambiguous WhatsApp delivery requires human review')}if(Number(error?.providerStatus)===429){await finish(claim,'RETRY',code).catch(()=>null);return {state:'RETRY',error:code}}if(error?.definitive===true||Number(error?.providerStatus)>=400&&Number(error?.providerStatus)<500){return requireHumanForFailure(claim,code,'WhatsApp delivery failed and needs human review')}if(PERMANENT_AI_FAILURES.has(code)){return requireHumanForFailure(claim,code,'Permanent AI or routing contract failure requires human review')}if(['AI_PLANNER_CONTRACT_INVALID','V3_INTERPRETER_CONTRACT_INVALID','V3_INTERPRETER_UNAVAILABLE'].includes(code)&&Number(claim?.attempt_count||0)>=2){return requireHumanForFailure(claim,code,'AI interpretation failed repeatedly')}if(Number(claim?.attempt_count||0)>=5){return requireHumanForFailure(claim,code,'AI processing exhausted safe retry attempts')}await finish(claim,'RETRY',code).catch(()=>null);return {state:'RETRY',error:code};}
...
export const _v3CutoverTest={engineForMode:mode=>mode==='canary'||mode==='active'?'V3':'LEGACY'};
