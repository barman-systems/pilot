...
function fastPath({context,previousState,raw}){
  ...
  if(pending?.purpose==='CONFIRM_TENTATIVE_VEHICLE'&&CONFIRM_RE.test(trimmed))return {proposal:{intent:'BOOKING',action:'REPLY',confidence:1,serviceName:null,serviceSurface:null,entities:[],serviceQuestion:null,dialogue:{message_role:'CONFIRMATION',evidence:trimmed,invalidated_fields:[]}},fastFacts};
  if(pending&&DENY_RE.test(trimmed))return {proposal:{intent:previousState?.goal==='BOOK_SERVICE'?'BOOKING':'SUPPORT',action:'REPLY',confidence:1,serviceName:null,serviceSurface:null,entities:[],serviceQuestion:null,dialogue:{message_role:'DENIAL',evidence:trimmed,invalidated_fields:[]}},fastFacts};
  const ordinal=trimmed.match(ORDINAL_RE);
  if(ordinal&&arr(pending?.options).length){const option=pending.options[Number(ordinal[1])-1];if(option?.type==='service'&&option.label)return {proposal:{intent:'BOOKING',action:'REPLY',confidence:1,serviceName:option.label,serviceSurface:trimmed,entities:[],serviceQuestion:null,dialogue:{message_role:'ANSWER_TO_PENDING_QUESTION',evidence:trimmed,invalidated_fields:[]}},fastFacts};}
  const exact=serviceExactFromMessage(context,trimmed);if(exact&&pending?.fields?.includes('service'))return {proposal:{intent:'BOOKING',action:'REPLY',confidence:1,serviceName:serviceLabel(exact),serviceSurface:trimmed,entities:[],serviceQuestion:null,dialogue:{message_role:'ANSWER_TO_PENDING_QUESTION',evidence:trimmed,invalidated_fields:[]}},fastFacts};
  const p=context?.pending_state,payload=p?.payload||{};
  if(ordinal&&p?.pending_action==='choose_slot'&&payload.presented===true&&arr(payload.slots).length>=Number(ordinal[1])){const index=Number(ordinal[1])-1,slot=payload.slots[index];fastFacts.push({field:'slot',value:index,source:'CUSTOMER_CONFIRMED',resolution:'PRESENTED_SLOT_SELECTION',confidence:1,starts_at:slot?.starts_at||null,service_id:slot?.service_id||null,worker_id:slot?.worker_id||null});return {proposal:{intent:previousState?.goal==='RESCHEDULE_BOOKING'?'RESCHEDULE_BOOKING':'BOOKING',action:previousState?.goal==='RESCHEDULE_BOOKING'?'RESCHEDULE_BOOKING':'CREATE_BOOKING',confidence:1,serviceName:null,serviceSurface:null,entities:[],serviceQuestion:null,dialogue:{message_role:'ANSWER_TO_PENDING_QUESTION',evidence:trimmed,invalidated_fields:[]}},fastFacts};}
  if(fastFacts.some(f=>f.field==='location')&&previousState?.goal==='BOOK_SERVICE')return {proposal:{intent:'BOOKING',action:'REPLY',confidence:1,serviceName:null,serviceSurface:null,entities:[],serviceQuestion:null,dialogue:{message_role:'ANSWER_TO_PENDING_QUESTION',evidence:trimmed||'location',invalidated_fields:[]}},fastFacts};
...
export async function interpretConversationTurnV3({context,previousState=null,generate=generateDABBIRAiReply,now=new Date()}){
  ...
  const proposal={intent:x.intent==='UNKNOWN'?'SUPPORT':x.intent,action:x.requested_action==='NONE'?'REPLY':x.requested_action,confidence:x.confidence,serviceName,serviceSurface:x.service_candidate?.surface||null,serviceCandidateLabel:x.service_candidate?.label||null,entities,
