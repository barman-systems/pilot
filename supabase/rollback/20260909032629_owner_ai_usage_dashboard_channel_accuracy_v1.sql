-- Restore the first owner AI usage snapshot definition.
...
as $function$
with bounds as (
  select pg_catalog.date_trunc('month', pg_catalog.now()) as month_start,
...
providers as (
  select coalesce(jsonb_agg(jsonb_build_object(
    'provider',p.provider,'ai_requests',p.ai_requests,'known_cost_aed',p.known_cost_aed,'unpriced_operations',p.unpriced_operations
...
select jsonb_build_object(
  ...
  'known_cost_per_conversation_aed',case when conversation.conversations>0 then round(cost.whatsapp_known_cost_aed/conversation.conversations,6) else null end,
