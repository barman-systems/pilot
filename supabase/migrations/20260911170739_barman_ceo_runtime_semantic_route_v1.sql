-- Preserve the existing runtime health lane as a semantic route chosen before execution.
...
begin
  ...
  if v_route not in ('REPO_CHANGE','DATA_QUERY','RUNTIME_CHECK','MULTI_STEP','EXTERNAL_ACTION','REVIEW_REQUIRED','OWNER_GATE') then raise exception 'EXECUTIVE_ROUTE_INVALID'; end if;
  ...
  v_lane:=case v_route when 'REPO_CHANGE' then 'tool_agent' when 'DATA_QUERY' then 'read_only' when 'RUNTIME_CHECK' then 'runtime' when 'MULTI_STEP' then 'planner' else null end;
