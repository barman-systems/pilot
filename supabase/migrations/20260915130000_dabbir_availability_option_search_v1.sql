...
as $function$
declare
  v_seed time;v_result jsonb;v_slot jsonb;v_slots jsonb:='[]'::jsonb;
  v_local text;v_time time;v_key text;v_seen text[]:=array[]::text[];
  v_limit integer:=least(12,greatest(1,coalesce(p_max_candidates,12)));
  v_state text:='NO_SLOTS';
begin
  if coalesce(auth.role(),'')<>'service_role' then raise exception 'SERVICE_ROLE_REQUIRED'; end if;
  if p_business_id is null or p_conversation_id is null or p_requested_date is null then raise exception 'AVAILABILITY_SEARCH_SCOPE_REQUIRED'; end if;
  if p_from is not null and p_to is not null and p_from>p_to then raise exception 'AVAILABILITY_SEARCH_RANGE_INVALID'; end if;

  -- Existing exact authority searches thirteen 30-minute points and returns up to
  -- three slots. Four deterministic seeds cover the local day, so twelve is the
  -- truthful maximum. Semantic phrases never become clock constants here.
  foreach v_seed in array array['00:00'::time,'06:30'::time,'13:00'::time,'19:30'::time] loop
    v_result:=public.dabbir_whatsapp_ai_check_availability(p_business_id,p_conversation_id,p_service_id,p_worker_id,p_requested_date::timestamp+v_seed);
    ...
    for v_slot in select value from jsonb_array_elements(coalesce(v_result->'slots','[]'::jsonb)) loop
      ...
      v_seen:=array_append(v_seen,v_key);v_slots:=v_slots||jsonb_build_array(v_slot);
  ...
  return jsonb_build_object('ok',true,'state',v_state,'requested_date',p_requested_date,'from',p_from,'to',p_to,'max_candidates',v_limit,'slots',v_slots,'read_only',true);
end;
$function$;
...
comment on function public.dabbir_whatsapp_ai_find_available_options_v1(uuid,uuid,uuid,uuid,date,time,time,integer) is 'Read-only candidate discovery over existing branch-scoped availability truth. Human semantic preferences are not translated to hidden clock ranges.';
