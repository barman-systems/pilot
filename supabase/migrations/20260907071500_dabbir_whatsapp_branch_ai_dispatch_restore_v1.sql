-- Restore durable WhatsApp AI queue/dispatch after branch-routing migration replaced
-- dabbir_whatsapp_persist_inbound without the AI enqueue block.
-- Keeps branch isolation while restoring the P0 receptionist fast path + cron recovery.
...
begin
  ...
  -- Fast path is best-effort. A durable batch remains for the recovery cron.
  begin
