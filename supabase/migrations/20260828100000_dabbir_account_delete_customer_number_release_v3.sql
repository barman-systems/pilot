-- DABBIR account deletion v3: reconcile immutable support number with verified account deletion.
-- The DAB-* account number remains immutable during normal operation. It may be
-- removed only after the same user has been atomically tombstoned as deleted.
...
begin
  ...
  -- Tombstone first. This is in the same database transaction: any later error
  -- rolls the tombstone back too. The tombstone authorizes the immutable DAB-*
  -- support-number guard to release only this deleted DABBIR identity and also
  -- triggers Apple entitlement / retained owner-reference cleanup.
...
comment on function dabbir_private.guard_dabbir_user_account_identity() is
  'Keeps DAB-* account identity immutable and undeletable during normal operation; permits deletion only after the same DABBIR user is atomically tombstoned deleted.';
