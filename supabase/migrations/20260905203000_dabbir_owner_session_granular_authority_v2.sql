-- Session authority payload for Phase 2 broker-side granular authorization.
...
begin
  ...
  where user_id=v_session.actor_user_id and active=true and revoked_at is null and suspended_at is null
    and role in ('ROOT_OWNER','OWNER_DELEGATE') and (access_expires_at is null or access_expires_at>now());
  if not found then
  ...
  return jsonb_build_object(
    'authenticated',true,'role','platform_owner','authority_role',v_admin.role,'root_owner',v_admin.role='ROOT_OWNER',
    'role_code',v_admin.role_code,
    'permissions',case when v_admin.role='ROOT_OWNER' then to_jsonb(array['manage_customers','manage_businesses','manage_orders','manage_bookings','manage_products','manage_services','manage_support','manage_incidents','manage_integrations','manage_employees','manage_system','manage_releases','manage_ceo_commands','view_financials','manage_financial_operations']::text[]) else to_jsonb(v_admin.permissions) end,
    'granular_permissions',case when v_admin.role='ROOT_OWNER' then (select coalesce(jsonb_agg(code order by code),'[]'::jsonb) from dabbir_private.platform_permissions) else to_jsonb(v_admin.granular_permissions) end,
    'display_name',v_admin.display_name,'actor_user_id',v_session.actor_user_id,'expires_at',v_session.expires_at,
    'access_expires_at',v_admin.access_expires_at,'access_scope',v_admin.access_scope,'mfa_required',v_admin.mfa_required,'approval_limit_aed',v_admin.approval_limit_aed
  );
end;
$$;
