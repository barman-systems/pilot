-- DABBIR platform-level account suspension.
-- Scope: DABBIR access only. Does not ban or mutate Supabase Auth users.

create table if not exists dabbir_private.account_access_state (
...
alter table dabbir_private.account_access_state enable row level security;
revoke all on dabbir_private.account_access_state from public, anon, authenticated;
grant all on dabbir_private.account_access_state to service_role;

drop policy if exists account_access_state_deny_clients on dabbir_private.account_access_state;
create policy account_access_state_deny_clients
on dabbir_private.account_access_state
for all
...
stable
security definer
set search_path = pg_catalog, dabbir_private, auth
as $$
  select (select auth.uid()) is not null
    and not exists (
      select 1
      from dabbir_private.account_access_state s
...
create or replace function public.dabbir_account_access_self()
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_state dabbir_private.account_access_state%rowtype;
begin
  ...
  select * into v_state
  from dabbir_private.account_access_state
  where user_id = v_user_id;

  if not found then
    return jsonb_build_object('status','active','reason',null,'suspended_at',null);
  end if;
...
revoke all on function public.dabbir_account_access_self() from public, anon;
grant execute on function public.dabbir_account_access_self() to authenticated, service_role;
...
-- Close direct-membership and direct-user RLS paths that bypass has_permission().
...
begin
  ...
  if v_status='suspended' then
    ...
    insert into dabbir_private.account_access_state(
...
begin
  ...
  with matches as (
    ...
    left join dabbir_private.account_access_state s on s.user_id=a.user_id
...
begin
  ...
  left join dabbir_private.account_access_state s on s.user_id=u.id
