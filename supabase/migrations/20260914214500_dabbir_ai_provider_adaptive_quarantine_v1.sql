-- DABBIR AI Provider Adaptive Quarantine V1
-- Data-driven follow-up to Provider Reliability Authority V1.
-- Keep provider priority unchanged; reduce repeated probes against providers that remain unhealthy.
...
begin
  ...
  if v_class in ('BILLING_CAPACITY_HARD_BLOCK','AUTH_CONFIGURATION') then
    ...
    on conflict(provider,model) do update set
      ...
      consecutive_failures=case when dabbir_private.dabbir_ai_provider_health_v1.failure_class=v_class and dabbir_private.dabbir_ai_provider_health_v1.last_observed_at>v_now-v_failure_memory then least(1000000,dabbir_private.dabbir_ai_provider_health_v1.consecutive_failures+1) else 1 end,
  ...
  if found and v_row.failure_class=v_class and v_row.last_observed_at is not null and v_row.last_observed_at>v_now-v_failure_memory then
...
comment on function public.dabbir_ai_provider_health_observe_v1(text,text,boolean,text,integer,integer,integer) is
  'Service-role-only adaptive provider health observation. Preserves provider order while progressively quarantining sustained 429/timeout/network/5xx failures; success resets immediately.';
