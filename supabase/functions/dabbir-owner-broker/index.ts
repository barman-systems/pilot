...
const permissions=(s:any)=>Array.isArray(s?.permissions)?s.permissions.map(String):[];
const can=(s:any,p:string)=>s?.authority_role==='ROOT_OWNER'||permissions(s).includes(p);
...
async function requestOtp(body:any){
 ...
 const identity=await resolveLogin(String(body?.login||body?.username||'__root__'));if(!identity)return reply(404,{ok:false,error:'PLATFORM_IDENTITY_NOT_FOUND'});
 ...
 const sent=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${resendKey}`,'content-type':'application/json'},body:JSON.stringify({from,to:[identity.email],subject:'DABBIR verification code',text:`رمز دخول دبّر: ${otp}\n\nينتهي الرمز خلال 10 دقائق.\nDABBIR verification code: ${otp}\nExpires in 10 minutes.`})});
...
async function verifyOtp(body:any){
 const id=clean(body?.challenge_id,80),otp=clean(body?.otp,10);if(!/^[0-9a-f-]{36}$/i.test(id)||!/^\d{6}$/.test(otp))return reply(401,{ok:false,error:'INVALID_OWNER_OTP'});
 ...
 if(row.invitation_id){const accepted=await rpc('dabbir_platform_staff_accept_for_user_v1',{p_user_id:row.actor_user_id});if(!accepted.ok)return reply(401,{ok:false,error:'INVITATION_ACCEPT_FAILED'})}
...
async function teamAction(session:any,body:any){
 const denied=requirePermission(session,'manage_employees');if(denied)return denied;
 const op=clean(body?.operation,40).toLowerCase();
 if(op==='list'){const r=await rpc('dabbir_platform_staff_list_v2',{p_actor:session.actor_user_id});return r.ok?reply(200,{ok:true,payload:r.payload}):reply(503,{ok:false,error:'TEAM_READ_FAILED'})}
 if(op==='invite'){
  const email=clean(body?.email,254).toLowerCase(),displayName=clean(body?.display_name,160),preset=clean(body?.preset,40)||'custom';
  const perms=Array.isArray(body?.permissions)?body.permissions.map((x:any)=>clean(x,80)).filter(Boolean):[];
  ...
  const raw=randomToken(40),expires=new Date(Date.now()+7*86400000).toISOString(),hash=await sha(`${SERVICE_KEY}:platform-invite:${raw}`);
  ...
  if(!r.ok)return reply(r.status===400?400:403,{ok:false,error:'TEAM_INVITE_FAILED'});
  const inviteId=r.payload?.id;
  const resendKey=clean(body?.resend_key,500);
  if(resendKey&&inviteId){const from=String(Deno.env.get('DABBIR_RESEND_FROM')||'DABBIR <onboarding@resend.dev>');const sent=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${resendKey}`,'content-type':'application/json'},body:JSON.stringify({from,to:[email],subject:'دعوة لإدارة DABBIR | دبّر',text:`تمت دعوتك كمدير بصلاحيات مستقلة في دبّر. افتح https://dabbir.cloud/owner واستخدم بريدك ${email} ثم رمز OTP المرسل لك.\n\nلا تشارك رمزك مع أي شخص.`})});await rpc('dabbir_platform_staff_invite_delivery_v1',{p_invitation_id:inviteId,p_status:sent.ok?'SENT':'FAILED'})}
  return reply(200,{ok:true,payload:r.payload});
 }
 if(['set_permissions','suspend','reactivate','revoke_sessions','remove'].includes(op)){
  const target=clean(body?.target_user_id,80);if(!UUID_RE.test(target))return reply(400,{ok:false,error:'INVALID_TARGET_USER'});
  const perms=Array.isArray(body?.permissions)?body.permissions.map((x:any)=>clean(x,80)).filter(Boolean):null;
  ...
  return r.ok?reply(200,{ok:true,payload:r.payload}):reply(403,{ok:false,error:'TEAM_UPDATE_DENIED'});
 }
 return reply(400,{ok:false,error:'UNKNOWN_TEAM_OPERATION'});
}
...
async function ownerData(body:any){
 ...
 if(action==='identity')return reply(200,{ok:true,payload:{authority_role:session.authority_role,permissions:session.permissions||[],root_owner:session.root_owner===true,display_name:session.display_name||null}});
 if(action==='team')return teamAction(session,body);
 if(action==='overview'){const denied=requireRoot(session);if(denied)return denied;const r=await rpc('dabbir_platform_owner_overview',{p_actor_user_id:session.actor_user_id});return r.ok?reply(200,{ok:true,payload:r.payload}):reply(503,{ok:false,error:'OWNER_DATA_FAILED'})}
 ...
 if(action==='search'){const denied=requirePermission(session,'manage_customers');if(denied)return denied;const r=await rpc('dabbir_platform_customer_search',{p_actor_user_id:session.actor_user_id,p_query:clean(body?.q,160),p_limit:50});const accounts=Array.isArray(r.payload)?r.payload:(Array.isArray(r.payload?.accounts)?r.payload.accounts:[]);return r.ok?reply(200,{ok:true,payload:{accounts}}):reply(503,{ok:false,error:'OWNER_SEARCH_FAILED'})}
 if(action==='incidents')return incidentRead(session,body);if(action==='incident_action')return incidentAction(session,body);
 if(action==='ceo_commands'){const denied=requirePermission(session,'manage_ceo_commands');if(denied)return denied;const r=await rpc('dabbir_ceo_commands_authorized_v1',{p_actor:session.actor_user_id,p_limit:Math.max(1,Math.min(Number(body?.limit)||30,50))});return r.ok?reply(200,{ok:true,payload:{commands:Array.isArray(r.payload)?r.payload:[]}}):reply(503,{ok:false,error:'CEO_COMMAND_READ_FAILED'})}
 if(action==='ceo_command_create'){const denied=requirePermission(session,'manage_ceo_commands');if(denied)return denied;const text=clean(body?.command_text,4000),priority=clean(body?.priority,4).toUpperCase()||'P1',objective=clean(body?.objective,1000)||null,due=body?.due_at?String(body.due_at):null,acceptance=Array.isArray(body?.acceptance_criteria)?body.acceptance_criteria.map((x:any)=>clean(x,500)).filter(Boolean).slice(0,20):[];if(text.length<4)return reply(400,{ok:false,error:'COMMAND_TEXT_INVALID'});if(!['P0','P1','P2','P3'].includes(priority))return reply(400,{ok:false,error:'PRIORITY_INVALID'});const r=await rpc('dabbir_ceo_command_create_authorized_v1',{p_actor:session.actor_user_id,p_command_text:text,p_priority:priority,p_objective:objective,p_acceptance_criteria:acceptance,p_due_at:due});return r.ok?reply(200,{ok:true,payload:{command:r.payload}}):reply(503,{ok:false,error:'CEO_COMMAND_CREATE_FAILED'})}
 if(action==='ceo_command_update'){const denied=requirePermission(session,'manage_ceo_commands');if(denied)return denied;const id=clean(body?.command_id,80),op=clean(body?.operation,30).toLowerCase();if(!UUID_RE.test(id))return reply(400,{ok:false,error:'INVALID_COMMAND_ID'});if(!['reprioritize','set_due_at','add_guidance','cancel','resume'].includes(op))return reply(400,{ok:false,error:'UNKNOWN_COMMAND_OPERATION'});const r=await rpc('dabbir_ceo_command_update_authorized_v1',{p_actor:session.actor_user_id,p_command_id:id,p_operation:op,p_priority:body?.priority||null,p_due_at:body?.due_at||null,p_guidance:clean(body?.guidance,2000)||null});return r.ok?reply(200,{ok:true,payload:{command:r.payload}}):reply(503,{ok:false,error:'CEO_COMMAND_UPDATE_FAILED'})}
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==='owner_session_verify'){const s=await verifySession(String(body?.session_token||''));return s?reply(200,{ok:true,authenticated:true,role:'platform_owner',authority_role:s.authority_role,permissions:s.permissions||[],root_owner:s.root_owner===true,display_name:s.display_name||null,actor_user_id:s.actor_user_id,expires_at:s.expires_at}):reply(401,{ok:false,authenticated:false,error:'OWNER_SESSION_REQUIRED'})}
