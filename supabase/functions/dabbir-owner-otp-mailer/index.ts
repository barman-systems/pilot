...
const ALLOWED_ISSUERS=new Set(['https://oidc.vercel.com',`https://oidc.vercel.com/${OWNER_SLUG}`]);
...
Deno.serve(async(req:Request)=>{
 ...
 try{await verifyVercelIdentity(bearer(req))}catch(error){console.warn('DABBIR_OWNER_MAILER_OIDC_REJECTED',String(error instanceof Error?error.message:error).slice(0,120));return reply(401,{ok:false,error:'OWNER_MAILER_UNAUTHORIZED'})}
