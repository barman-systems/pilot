...
async function deleteQaUser(userId:string,runId:string){await getQaUser(userId,runId);const {error}=await db.auth.admin.deleteUser(userId);if(error)throw new Error(`AUTH_USER_DELETE_FAILED:${error.message}`);return {deleted:true,user_id:userId}}
