...
import { checkPasswordCompromise, passwordHashRange, PASSWORD_BREACH_GUARD_VERSION } from '../api/_password-breach-check.js';
...
test('Pwned Passwords range request uses k-anonymity and padding', async () => {
  ...
  assert.match(observed.options.headers['user-agent'], new RegExp(PASSWORD_BREACH_GUARD_VERSION));
});
