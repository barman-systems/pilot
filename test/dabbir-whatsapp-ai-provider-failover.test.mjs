...
test('all supported AI provider-chain failures are classified for immediate continuity failover',()=>{
  for(const code of ['gateway_http_404','gateway_timeout','gemini_http_429','groq_network_error','cloudflare_timeout','AI_PLANNER_UNAVAILABLE','empty_ai_response']){
...
test('fast WhatsApp dispatch converts exhausted provider retry into HUMAN_REQUIRED immediately',()=>{
...
test('recovery cron drains provider-failure retries so an interrupted fast dispatch cannot stay silent',()=>{
...
test('database failover is service-role only, stale-turn safe, atomic, and terminal',()=>{
