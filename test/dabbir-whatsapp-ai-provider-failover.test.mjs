...
const repair=fs.readFileSync(new URL('../supabase/migrations/20260908081500_dabbir_whatsapp_ai_continuity_root_v1.sql',import.meta.url),'utf8');
...
test('recovery cron still drains provider failures but ordinary degradation no longer creates a handoff',()=>{
  ...
  assert.match(repair,/no handoff and no conversation action_required transition/i);
  ...
  assert.doesNotMatch(providerFn,/set state='HUMAN_REQUIRED'/i);
});
...
test('database continuity is service-role-only, stale-turn safe, and closes only after verified Meta acceptance',()=>{
  ...
  assert.match(repair,/set search_path to ''/i);
  ...
  for(const fn of ['dabbir_whatsapp_ai_provider_failover\\(uuid,text\\)','dabbir_whatsapp_ai_provider_degraded_complete\\(uuid,text\\)','dabbir_whatsapp_ai_provider_degraded_handoff\\(uuid,text\\)']){
    assert.match(repair,new RegExp(`revoke all on function public\\.${fn} from public,anon,authenticated`,'i'));
  }
});

test('generic handoff metadata distinguishes a real customer human request from system escalation',()=>{
