...
test('aal1 session with verified TOTP requires MFA and exposes only safe factor metadata', async () => {
  const mock = installFetch({
    factors: [{ id: FACTOR_ID, factor_type: 'totp', status: 'verified', friendly_name: 'DABBIR Authenticator', secret: 'MUST_NOT_LEAK' }],
  });
  try {
    ...
    assert.equal(result.body.factor_id, FACTOR_ID);
    assert.equal(result.body.factor_type, 'totp');
    assert.deepEqual(result.body.factors, [{ id: FACTOR_ID, factor_type: 'totp', friendly_name: 'DABBIR Authenticator' }]);
...
test('aal2 session does not request another MFA challenge', async () => {
  const mock = installFetch({
    factors: [{ id: FACTOR_ID, factor_type: 'totp', status: 'verified', friendly_name: 'DABBIR Authenticator' }],
  });
  try {
...
test('unverified factors never force an MFA continuation', async () => {
  const mock = installFetch({
    factors: [{ id: FACTOR_ID, factor_type: 'totp', status: 'unverified', friendly_name: 'Pending' }],
  });
  try {
    const result = await invoke(token('aal1'));
    assert.equal(result.status, 200);
