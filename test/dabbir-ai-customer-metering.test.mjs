...
class SharedProviderStore{
  ...
  async claim(provider,model){
    const row=this.rows.get(this.key(provider,model));
    if(row?.cooldownUntil>Date.now())return {decision:'SKIP',reason:'PROVIDER_COOLDOWN',failure_class:row.failureClass,cooldown_remaining_ms:row.cooldownUntil-Date.now(),health_state:'SHARED'};
    return {decision:'ATTEMPT',health_state:'SHARED'};
  }
  async observe(provider,model,observation){
    if(observation.success){this.rows.delete(this.key(provider,model));return {cooldown_ms:0};}
    if(observation.failureClass==='RATE_LIMIT'){
      const duration=observation.retryAfterMs||60_000;
      this.rows.set(this.key(provider,model),{failureClass:'RATE_LIMIT',cooldownUntil:Date.now()+duration});
      return {cooldown_ms:duration};
    }
...
test('synthetic provider telemetry preserves unknown cost and reports only actual token evidence',async()=>{
 for(const usage of [undefined,{prompt_tokens:12,completion_tokens:7},{prompt_tokens:null,completion_tokens:7}]){
  ...
  assert.equal(r.telemetry.provider_reliability.network_attempts,1);
  assert.doesNotMatch(JSON.stringify(r.telemetry),/secret-test|https:|مرحبا/);
 }
});

test('shared 429 cooldown skips a known-limited direct provider across instances without changing provider priority',async()=>{
  const calls=[];
  const sharedStore=new SharedProviderStore();
  ...
  const first=await generateDABBIRAiReply({project:'dabbir_businesses',message:'first',env,fetchImpl,providerHealthStore:sharedStore});
  assert.equal(first.ok,true);
  assert.deepEqual(first.telemetry.attempts.map(x=>`${x.provider}:${x.status}`),['google-gemini:429','groq:200']);
  const firstGemini=first.telemetry.provider_reliability.attempts.find(x=>x.provider==='google-gemini');
  assert.equal(firstGemini.failure_class,'RATE_LIMIT');
  assert.equal(firstGemini.cooldown_applied_ms,60_000);
  ...
  assert.ok(second.telemetry.skipped_attempts.some(x=>x.provider==='google-gemini'&&x.reason==='PROVIDER_COOLDOWN'&&x.retry_after_ms>0));
  assert.equal(second.telemetry.provider_reliability.provider_attempts_saved,1);
});
...
const reliability=fs.readFileSync(new URL('../api/_ai-provider-reliability.js',import.meta.url),'utf8');
...
test('meter preserves paid fallback attribution while provider health belongs to the shared authority',()=>{
  ...
  assert.match(meter,/provider_reliability/);
  assert.match(meter,/skipped_attempts/);
  assert.doesNotMatch(meter,/providerCooldowns|provider429Strikes|cooldownKey|armProviderCooldown/);
  assert.match(reliability,/dabbir_ai_provider_health_claim_v1/);
  assert.match(reliability,/dabbir_ai_provider_health_observe_v1/);
  assert.match(reliability,/retry-after/);
