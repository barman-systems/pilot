...
test('index failures expose bounded provider codes without knowledge text or credentials',async()=>{
 const row={knowledge_id:'22222222-2222-4222-8222-222222222222',business_id:'11111111-1111-4111-8111-111111111111',content:'PRIVATE CUSTOMER CONTENT',content_hash:'0123456789abcdef0123456789abcdef'};
...
test('indexer embeds only rows supplied by approved-knowledge queue and does not need a new credential',async()=>{const calls=[];const result=await indexApprovedKnowledge({env:{...enabledEnv,GEMINI_API_KEY:'same-key'},fetchImpl:async()=>({ok:true,status:200,text:async()=>JSON.stringify({embedding:{values:vector}})}),rpc:async(name,p)=>{calls.push([name,p]);if(name==='dabbir_knowledge_embedding_queue_v1')return [{knowledge_id:'22222222-2222-4222-8222-222222222222',business_id:'11111111-1111-4111-8111-111111111111',title:'policy',content:'owner approved',content_hash:'0123456789abcdef0123456789abcdef'}];return {ok:true,knowledge_id:p.p_knowledge_id,business_id:'11111111-1111-4111-8111-111111111111',content_hash:p.p_content_hash}}});assert.equal(result.indexed,1);assert.equal(calls[1][0],'dabbir_knowledge_embedding_upsert_v1');assert.equal(calls[1][1].p_model,'gemini-embedding-2')});
...
test('indexer preserves database content bytes and checksum across multilingual whitespace',async()=>{
 ...
 const row={knowledge_id:'22222222-2222-4222-8222-222222222222',business_id:'11111111-1111-4111-8111-111111111111',title:'approved policy',content,content_hash:createHash('md5').update(content).digest('hex')};
 let writes=0;
 ...
 assert.equal(writes,1);assert.equal(result.indexed,1);assert.equal(result.failed,0);assert.equal(result.ok,true);
});
test('indexer does not claim success for an unverified database result',async()=>{
 const row={knowledge_id:'22222222-2222-4222-8222-222222222222',business_id:'11111111-1111-4111-8111-111111111111',content:'approved policy',content_hash:'0123456789abcdef0123456789abcdef'};
 const result=await indexApprovedKnowledge({env:enabledEnv,fetchImpl:async()=>({ok:true,status:200,text:async()=>JSON.stringify({embedding:{values:vector}})}),rpc:async name=>name==='dabbir_knowledge_embedding_queue_v1'?[row]:{ok:true,knowledge_id:row.knowledge_id,business_id:'33333333-3333-4333-8333-333333333333',content_hash:row.content_hash}});
