...
test('billing reconciliation is a protected bounded cron with a settled rolling window',()=>{const cfg=JSON.parse(vercel);const item=cfg.crons.find(item=>item.path==='/api/dabbir-ai-billing-reconcile-cron');assert.ok(item);assert.ok(['23 2 * * *','*/5 * * * *'].includes(item.schedule));assert.equal(cfg.functions['api/dabbir-ai-billing-reconcile-cron.js'].maxDuration,60);assert.match(cron,/CRON_SECRET/);assert.match(cron,/slice\(0,500\)/);assert.match(cron,/SETTLEMENT_LAG_DAYS=2/);assert.match(cron,/RECONCILE_DAYS=3/);assert.match(cron,/settlement_lag_days:SETTLEMENT_LAG_DAYS/)});
