...
test('Google Play subscription is verified server-side and bound to DABBIR UUID', async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (String(url) === 'https://oauth2.googleapis.com/token') return responseJson({ access_token: 'server-test-access-token', expires_in: 3600 });
    if (String(url).includes('/purchases/subscriptionsv2/tokens/')) return responseJson(activeSubscription());
...
test('Google Play verification rejects malformed purchase tokens before provider access', async () => {
  let called = false;
  globalThis.fetch = async () => { called = true; throw new Error('SHOULD_NOT_CALL'); };
  await assert.rejects(
    () => verifyGoogleSubscription('short', USER_ID),
    error => error?.message === 'GOOGLE_PLAY_PURCHASE_TOKEN_INVALID' && error?.code === 400,
  );
