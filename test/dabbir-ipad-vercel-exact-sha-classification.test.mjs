...
function changePath(dir,relativePath){
  ...
  fs.writeFileSync(target,relativePath.endsWith('.yml')?'name: canonical ipad production\n':'export {};\n');
  git(dir,'add','.');
...
test('every canonical iPad exact-SHA QA contract forces a Production deployment instead of Vercel ignore',()=>{
  for(const relativePath of [
    ...
    'test/dabbir-ipad-vercel-exact-sha-classification.test.mjs',
  ]){
    const dir=setupRepo();
