import {captureBeforeUse} from './s07-evidence-gate.mjs';
import {interpretConversationTurnV3} from '../B/api/_dabbir-conversation-v3-interpreter.js';
import {classifyEpisodeBoundaryV3} from '../B/api/_dabbir-conversation-v3-episode.js';
import {seedConversationStateV3,freshConversationStateV3,understandTurnV3} from '../B/api/_dabbir-conversation-v3-understanding.js';
import {planConversationTurnV3} from '../B/api/_dabbir-conversation-v3-brain.js';
import {assertFinalResponseSourceV3,brainResponseV3,V3_RESPONSE_SOURCE} from '../B/api/_dabbir-conversation-v3-invariants.js';
import {verifiedAvailability,assertResponseGrounding} from '../B/api/_dabbir-brain-contract.js';
...
export async function runConversationV3Runtime({claim,context,rpc,deliver,finish,handoff,bookingText,slotsText,interpreter=interpretConversationTurnV3,now=()=>new Date(),logger=console,preloadedLoad=null,evidence=null}){
  const load=structuredClone(preloadedLoad||await rpc('dabbir_semantic_load_v2',{p_batch_id:claim.batch_id,p_lock_token:claim.lock_token}));
  await captureBeforeUse(load, evidence);
