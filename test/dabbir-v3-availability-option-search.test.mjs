...
test('broad availability migration satisfies canonical deploy contract',()=>{
  const x=validateMigrationFile(path.join(root,relative),{cutoverVersion:'20260915124900'});
  assert.ok(x.bytes>0);assert.ok(x.statementCount>0);
});
...
test('four exact-authority seeds cover the day without semantic daypart constants',()=>{
  for(const seed of ['00:00','06:30','13:00','19:30'])assert.ok(sql.includes(`'${seed}'::time`));
  assert.doesNotMatch(sql,/EARLY_MORNING|MORNING|AFTERNOON|EVENING|NIGHT/);
});

test('customer hard time bounds and result volume are enforced by code',()=>{
  assert.match(sql,/p_from is not null and v_time<p_from/);
  assert.match(sql,/p_to is not null and v_time>p_to/);
  assert.match(sql,/least\(24,greatest\(1,coalesce\(p_max_candidates,12\)\)\)/);
  assert.match(sql,/left\(v_local,10\)<>p_requested_date::text/);
});
