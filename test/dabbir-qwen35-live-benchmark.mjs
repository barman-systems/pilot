...
const ORIGIN=String(process.env.QWEN35_BENCHMARK_ORIGIN||'').trim().replace(/\/$/,'');
...
const RUN_ID=`qwen35-benchmark-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;
const REPORT_PATH=String(process.env.QWEN35_BENCHMARK_REPORT_PATH||'dabbir-qwen35-live-benchmark.json');
const SCENARIOS=['critical','service_details','correction_side_question','multiple_requests','context_references'];
const BENCHMARK_SCOPE='qwen35-live-v1';

if(!/^https:\/\/[^/]+$/i.test(ORIGIN))throw new Error('QWEN35_BENCHMARK_ORIGIN_REQUIRED');
...
async function run(){
 ...
 for(const scenario of SCENARIOS){
  const r=await request('/api/dabbir-qwen35-benchmark',{method:'POST',headers:{'x-dabbir-benchmark-scope':BENCHMARK_SCOPE},body:{synthetic:true,scenario},timeoutMs:120000});
...
try{await run();}
 ...
 console.log(`DABBIR_QWEN35_LIVE_BENCHMARK=${report.verdict}`);
