...
test('grounded duration side question outranks unresolved pending time even when provider role is misclassified',async()=>{
 ...
 h.state.intent_confirmed=true;
 h.state.pending_action='CLARIFY';h.state.clarification_entity='time';h.state.missing_fields=['time'];
 h.state.cognition={...(h.state.cognition||{}),pending_field:'time',pending_question:{field:'time',text:'أي وقت يناسبك؟',presentation:'PENDING_DELIVERY'}};
