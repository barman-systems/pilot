...
test('every cron handler rejects spoofed scheduler identity before any external call',async()=>{
 ...
 const handlers=[reminder,calendar,whatsapp,daily,billing,intelligence,composio,executive];
 try {
  ...
  for(const secret of [undefined,'configured-cron-secret']){
   ...
   for(const handler of handlers)for(const schedule of ['*/5 * * * *','15 5 * * *','23 2 * * *','17 * * * *']){
