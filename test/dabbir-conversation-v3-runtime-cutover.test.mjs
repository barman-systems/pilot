...
test('V3 interpreter semantically maps Arabic-spelled VIP to a scoped candidate but does not grant authority',async()=>{
  ...
  assert.equal(out.proposal.serviceName,'Vip');assert.equal(out.proposal.serviceSurface,'في اي بي');assert.equal(out.proposal.serviceVerified,false);assert.equal(out.proposal.intent,'BOOKING');
});

test('V3 fast path resolves an offered service ordinal without calling a model and marks it verified',async()=>{
  ...
  assert.equal(calls,0);assert.equal(out.proposal.serviceName,'Vip');assert.equal(out.proposal.serviceVerified,true);assert.equal(out.provider,'deterministic-v3-fast-path');
});

test('typed text cannot become a verified GPS fact without a signed location receipt',()=>{
  const noReceipt=_v3InterpreterTest.fastPath({context:context('هذا موقعي'),previousState:{goal:'BOOK_SERVICE'},raw:'هذا موقعي'});assert.equal(noReceipt.fastFacts.some(x=>x.field==='location'),false);
  ...
  const receipt=_v3InterpreterTest.fastPath({context:c,previousState:{goal:'BOOK_SERVICE'},raw:'location'});assert.equal(receipt.fastFacts.find(x=>x.field==='location')?.source,'PROVIDER_VERIFIED');
});

test('AI core uses the V3 json schema when semantic=v3',async()=>{
  let requestBody=null;const fetchImpl=async(_url,options)=>{requestBody=JSON.parse(options.body);return {ok:true,status:200,json:async()=>({model:'openai/gpt-oss-20b',choices:[{message:{content:JSON.stringify(valid({intent:'SUPPORT',role:'GREETING',confidence:1}))},finish_reason:'stop'}]}),clone(){return this},headers:{get:()=>null}}};
...
test('real V3 runtime writes and sends the Brain response without legacy dialogue ownership',async()=>{
  const c=context('مرحبا');const calls=[],sent=[],logs=[];const load={version:0,message_revision:1,semantic_state:{},cognitive_policy:{mode:'canary'},activity_profile:c.activity_profile};
  ...
  const interpreter=async()=>({proposal:{intent:'SUPPORT',action:'REPLY',confidence:1,serviceName:null,serviceSurface:null,serviceVerified:false,entities:[],serviceQuestion:null,dialogue:{message_role:'GREETING',evidence:'مرحبا',invalidated_fields:[]}},fastFacts:[],provider:'stub-v3',model:'stub'});
...
test('semantic service match stays tentative in authority state and asks for one grounded confirmation',async()=>{
  const c=context('في اي بي'),sent=[];let committedState=null;
  const previous={version:2,goal:'BOOK_SERVICE',intent_confirmed:true,facts:[{field:'delivery_mode',status:'VERIFIED',value:'MOBILE',source:'DATABASE_FACT',confidence:1}],tentatives:[],pending_question:{fields:['service'],purpose:'COLLECT_SERVICE',options:services.map(s=>({type:'service',id:s.id,label:s.name,price:s.price,currency:'AED'}))},last_turn_at:'2026-09-11T07:59:30Z',episode_id:'e1',episode_started_at:'2026-09-11T07:59:00Z'};
  const load={version:4,message_revision:5,semantic_state:{v3_runtime:previous},cognitive_policy:{mode:'canary'},activity_profile:c.activity_profile};
  const rpc=async(name,args)=>{if(name==='dabbir_semantic_commit_v2'){committedState=args.p_state;return {version:5,replay:false,state:args.p_state}}if(name==='dabbir_semantic_assert_current_v2'||name==='dabbir_record_ai_operator_decision_v1')return true;throw new Error(`unexpected rpc ${name}`)};
  const interpreter=async()=>({proposal:{intent:'BOOKING',action:'REPLY',confidence:.97,serviceName:'Vip',serviceSurface:'في اي بي',serviceCandidateLabel:'Vip',serviceVerified:false,entities:[],serviceQuestion:null,dialogue:{message_role:'ANSWER_TO_PENDING_QUESTION',evidence:'في اي بي',invalidated_fields:[]}},fastFacts:[],provider:'stub-v3',model:'stub'});
  await runConversationV3Runtime({claim:{batch_id:c.batch.id,lock_token:'80000000-0000-4000-8000-000000000001'},context:c,rpc,deliver:async(_claim,_ctx,body)=>{sent.push(body);return {providerMessageId:'meta-v3'}},finish:async()=>true,handoff:async()=>{throw new Error('no handoff')},bookingText:()=>'',slotsText:()=>'',interpreter,preloadedLoad:load});
  assert.equal(committedState.entities.service,undefined);assert.equal(committedState.v3_runtime.tentatives.find(x=>x.field==='service')?.candidate_value,ids.vip);assert.equal(committedState.v3_runtime.pending_question?.purpose,'CONFIRM_TENTATIVE_SERVICE');assert.match(sent[0],/Vip/);assert.match(sent[0],/100 درهم/);assert.equal((sent[0].match(/[؟?]/g)||[]).length,1);
});

test('authority projection never promotes tentative vehicle into executable canonical entities',()=>{
  const c=context('جيب شيروكي'),state={version:2,goal:'BOOK_SERVICE',intent_confirmed:true,episode_id:'e',episode_started_at:c.batch.last_message_at,last_turn_at:c.batch.last_message_at,episode_boundary:{kind:'CONTINUE'},facts:[{field:'service',status:'VERIFIED',value:ids.vip,source:'CUSTOMER_CONFIRMED',confidence:1},{field:'delivery_mode',status:'VERIFIED',value:'MOBILE',source:'DATABASE_FACT',confidence:1}],tentatives:[{field:'vehicle',status:'TENTATIVE',candidate_value:'station',surface:'جيب شيروكي'}],invalidations:[],pending_question:null};
...
test('unmapped service language is preserved as tentative instead of disappearing',()=>{
  const base={facts:[],tentatives:[],turn_tentative:[]};const out=_v3RuntimeTest.preserveUnmappedService(base,{serviceName:null,serviceSurface:'غسيل الشخصيات',confidence:.72});const service=out.tentatives.find(x=>x.field==='service');
