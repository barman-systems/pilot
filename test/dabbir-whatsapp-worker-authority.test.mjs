...
function worker({ text = 'الثاني', failure = null } = {}) {
  const calls = [];
  ...
  const claim = { batch_id: 'fixture-batch', lock_token: 'fixture-lock', attempt_count: 1 };
  global.fetch = async (url, init) => {
    ...
    let result;
    if (name === 'dabbir_whatsapp_ai_context') result = ctx;
  ...
  return { calls, run: () => processClaimedWhatsAppAiBatch(claim) };
}
